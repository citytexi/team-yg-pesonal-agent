...
## [2026-07-29] ingest | 닉네임 생성 정책 2건(S-002 앱·S-102 그룹, 날짜·작성자 미기재) — 판본 대체 없음. 두 문서의 §2~3 유효성·예시는 기존 문서의 중복 전재이고 고유 내용은 §1(생성)뿐이라, 유효성 정본은 S-102-닉네임-정책-v0.2 유지하고 생성 정본만 신규 2건이 갖는 역할 분담으로 처리(파일명도 '닉네임-생성-정책' 계열로 분리). 신규 개념 [[닉네임-자동-생성]] 생성 — 조합 풀 전문 전재(형용사/동사 100 × 파르페 재료 30 = 3000), 계정 생성 1회·즉시 DB 저장·재계산 없음, 앱↔그룹 초기값은 계정 공통 1개 값 재사용(그룹마다 재추첨 안 함), 랜덤 수명 3종 구분(닉네임 영구 고정 / 토핑 템플릿 첫 토핑까지 고정 / 변형 타입 매 조회 재추첨). open-questions 2건 해소: 단어 목록 미수집(풀 수록으로 해소) / 자동 생성물 15자 초과 가능성(풀 실측 최대 11자 = 형용사 최장 바스락거리는 6 + 재료 최장 슈가파우더 5 → 구조적으로 불가). 2건 신규: [K2] 생성 주체 서버 vs 클라이언트 / 앱 닉네임 수정 시 그룹 닉네임 연동 여부(데이터 모델 결정 사안). 기록: S-102 생성 정책 §4가 참조하는 S-002-앱닉네임-정책-v0.3은 실재하지 않는 판본. 갱신: 이름-입력-규칙(자동 생성 절 → 관계 서술로 축약, 생성 상세는 신규 개념으로 이관)·overview·index

## [2026-07-29] ingest | S-002 앱닉네임 정책 v0.2 — 팀 확인 결과 S-002도 S-102 v0.2와 내용 동일하다는 답을 받아 v0.2를 화면별 판본으로 반영. S-002-앱닉네임-정책-v0.1은 superseded(superseded_by v0.2). 내용은 S-102 v0.2와 문자 그대로 동일(유효성 3종 + 초기 자동 생성 규칙), 적용 화면만 다름. **문서를 합치지 않고 계열 유지** — 두 화면이 나중에 갈라져도 각자 분기칠 수 있게 하려는 의도(사용자 지시). 이름-입력-규칙 상한 표의 출처를 화면별 v0.2로 정리하고 "화면별 문서 분리 유지" 근거를 명시. 갱신: 이름-입력-규칙·overview·index

## [2026-07-29] lint | 점검 완료 — 기계 검사(lint.py 67건·check-status.py 36건) 위반 0건, 민감 데이터 0건, 신규 모순 0건(당일 ingest 중 5건 등록 완료). stale 7건 수정: sources/S-102-닉네임-정책-v0.2 3건(S-002 v0.1 current 유지 서술·단어 풀 미수집·미해결 2건 → 모두 해소 반영), sources/S-102-그룹-닉네임-생성-정책-v0.1·S-002-앱-닉네임-생성-정책-v0.1의 S-002 v0.1 참조 → v0.2, concepts/그룹의 닉네임 정본 폐기본 2개 인용 → v0.2 2개 교체 + 초기값 계정 공통 재사용 서술 추가, concepts/그룹의 조회·정렬 근거 폐기본(설계서 v0.2) → v0.3. 패턴 기록: 같은 날 여러 판본을 연속 ingest하면 앞 회차의 "현행" 서술이 뒤 회차에서 폐기본 인용으로 바뀐다. 데이터 공백 5건(툴팁 문구·조회 실패 에셋·토핑-정책 v0.2 미수집·금칙어/중복·참조명 불일치 2건). 보고서 wiki/synthesis/lint-2026-07-29.md
...
## [2026-08-31] ingest | 공통 로딩 v0.1 (화면 비종속 로딩 애니메이션 — Lottie 2종·44x44·60fps·2초 loop)
## [2026-08-31] ingest | 스플래시 정책 A-001 v0.1 (로고 Lottie 3.5초 타임라인·최소 노출 시간 보장)
## [2026-09-03] sync | 디자인 파일에서 `G-001-Error` 프레임 2종(새로고침 중·조회 실패) 확인 — 미수집 문서가 아니라 디자인 프레임이었다. open-questions 3건 갱신: OQ-W-014 ③ 부분 확인, OQ-W-029 부분 진전(Pull-to-Refresh = 다크 로띠·딤 없음·목록 비움 확정, 초기 로딩은 여전히 미기재), OQ-W-033 신설(로딩·실패 문구가 디자인에만 있고 정책 문서에 없다 — Android 구현이 먼저 문자열로 굳혔다)
