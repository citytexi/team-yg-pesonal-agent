...
- **상태**: 미해결 (**디자인 시안 대기** — PR6는 시각 장치를 두지 않고 `ContentScale.Fit`만 적용한다)
- **해소 메모**: PR6가 `Fit`을 고른 것은 잘림을 막는 최소 조치이지 표시 정책의 확정이 아니다. 위키에
  갤러리 최근 섹션의 정책 근거가 없어 이 항목이 그 자리를 대신 지킨다.
