...
- **해소 메모**: ①은 이번 라운드로 닫혔고(도달 불가 기간은 약 5일이었다), ③은 pull-to-refresh 하나로 확정됐다.
  ...
  > 🔁 **미머지 브랜치가 한 번 더 뒤집는다(2026-09-03)** — 갈림의 기준이 "목록이 남아 있는가"에서
...
- **상태**: 미해결 (① ②는 디자인 입력 대기, ③은 위키 ingest 판단)
  ...
  > 📌 **①과 ③이 같은 미머지 브랜치에서 답을 얻었다(2026-09-03)** — 디자인이 두 상태를 각각
...
- **상태**: 미해결 (**브랜치 머지 대기** — 머지 전에는 스펙을 고치지 않는다,
  [2026-07-13] 항목이 정한 규율)
- **해소 메모**: 이 브랜치가 건드리는 미결이 셋 더 있다 — OQ-P-112(실패 = 전면 교체 규칙이
  또 뒤집힌다), OQ-P-113(①·③이 답을 얻는다), OQ-P-167(G-001이 토스트를 도로 내놓아 "세 번째
  화면"이 사라진다). 각 항목에 메모를 달아 뒀으니 머지 회차에 함께 본다.
  **함께 발견한 결함은 OQ-P-349로 갈랐다** — 이 브랜치가 G-001에서만 우회했을 뿐 남아 있다.
