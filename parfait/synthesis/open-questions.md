...
- **상태**: 부분 해소 (③④ 해소 — 2026-08-16 PR #261 / ①② 잔존: 문구·배치 근거 부재)
...
- **상태**: 미해결 (**두 번째 화면이 같은 자리에서 갈렸고, 이번에는 상수가 아니라 축이 다른 비교다**)
...
- **해소 메모**: ①은 절대경로 전용 `readFileBytes(path)`를 따로 두고 종류가 읽기 경로를 가르는 것으로,
  ...
  "사진 편집"을 잠근다. 셋을 다 저장하면 용량이 3배가 되면서 `MAX_SIZE = 9`와 정면으로 부딪히고,
...
- **상태**: 미해결 (**실기기 1회로 갈린다**)
- **해소 메모**: ②가 "안 찬다"로 판명되면 폴백을 걷거나, 후보가 0건일 때만 전경 마스크 옵션으로
  2차 세그멘테이션을 돌리는 쪽으로 바꾼다. 후자는 비용을 실제로 필요한 경우에만 지불한다.
...
- **상태**: 미해결 (**①②는 문구가 아니라 흐름 결정이다** — 코드 자리는 `ShowError`로 이미 있다)
- **해소 메모**: 이 화면의 실패 표현은 `CanvasBGEditError` enum 3종 + 토스트 하나뿐이라
  ②(부분 실패)를 담을 모양이 없다. OQ-P-270 ①과 한 라운드에서 정하는 편이 낫다 — 둘 다 같은
  화면·같은 도메인의 실패이고 코드 자리도 같다. ③은 OQ-P-261 ②(마감 409 판정 공용화)와 맞물린다.
  정하면 [c301-topping-edit-tab 스펙](../specs/archive/2026-08-16-c301-topping-edit-tab.md)
  as-built 절의 비대칭 서술과 [c301 배경 스펙](../specs/archive/2026-08-15-c301-canvas-background-edit.md)
  「실패 표현」을 함께 고친다.
