...
- **테두리를 그리는 화면 셋이 같은 렌더러를 쓴다.** 알맹이 위에 8방향 스탬프를 얹어 그리고,
  `CanvasToppingLayer`의 스탬프 컴포저블을 추출해 공유한다. 대상은 배치 화면과 캔버스만이 아니라
  그 사이의 **누끼 확인 화면**도 포함한다 — 지금 그 화면은 구운 PNG를 그대로 띄우므로, 바꾸지
  않으면 사용자가 한 흐름 안에서 같은 테두리를 편집·확인(구운 굵기)과 배치·캔버스(화면 dp 고정)의
  **두 가지 굵기로** 보게 된다. 캔버스 토핑의 긴 변이 Canvas-Area 너비의 일부에 불과해 차이가 작지 않다.
...
- 캐시 PNG가 흐름당 한 장 더 는다(트리밍된 알맹이). OQ-P-228의 메모리 압박과 같은 축이다.
