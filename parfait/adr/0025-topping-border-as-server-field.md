...
- **배치 화면 미리보기도 같은 렌더러로 바꾼다.** 알맹이 위에 8방향 스탬프를 얹어 그린다.
  캔버스와 다른 방식으로 그리면 "본 대로 올라간다"가 깨진다. `CanvasToppingLayer`의 스탬프
  컴포저블을 두 화면이 공유하도록 추출한다.
...
- 서버 계약·읽기 렌더러·`updateToppingBorder`가 처음으로 실제 쓰인다.
- C-301 테두리 재편집이 필드 수정 한 번으로 끝난다(이미지 재업로드 불필요).
- 같은 알맹이를 여러 테두리로 재사용할 여지가 생긴다(서버 `referenceCount`의 전제와도 맞는다).

**트레이드오프**

- 캐시 PNG가 흐름당 한 장 더 는다(트리밍된 알맹이). `segmentation-pipeline-hardening`이 2장으로
  늘린 데 이어 3장이다 — OQ-P-228의 메모리 압박과 같은 축이다.
