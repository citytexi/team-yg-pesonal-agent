...
**Goal:** 4개 외부 repo(android/skills·Kotlin/kotlin-agent-skills·skydoves/compose-performance-skills·chrisbanes/skills)의 74개 스킬을 이 repo `.claude/skills/`에 벤더링하고, 경량 검색 스킬(`skill-finder`)과 baseline+diff 업데이트 스킬(`update-injected-skills`)로 탐색·유지보수를 배선한다.
...
**Spec:** [specs/2026-07-22-vendor-android-kotlin-skills.md](../specs/2026-07-22-vendor-android-kotlin-skills.md)
...
Expected: `4 repos 74 skills` (± upstream 변동). 파일 미생성(dry).
...
### Task 7: 실제 전량 벤더 실행 (74 스킬 설치)

**Files:**
- Create: `.claude/skills/<74 vendored>/` (스크립트 산출)
...
Expected: `[full] 4 repos, 74 skills` (± upstream).
...
Expected: 스킬 dir 74 + 기존 스킬, licenses 4개, manifest 74.
...
git commit -m "feat(skills): 74개 android/kotlin/compose 스킬 전량 벤더링 + baseline/manifest/catalog/licenses"
```
...
- 열린 질문(스펙): 랭킹은 키워드 가중 채택(BM25 미도입, 74 규모 충분). CATALOG 그룹은 repo/topic 자동 — 후속 수동 조정 여지. 충돌 접두사는 Task 7 Step 3에서 검증(현재 0).
