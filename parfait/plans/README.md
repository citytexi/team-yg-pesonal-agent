...
| 계획 | 내용 |
|------|------|
| [2026-07-21-app-preview-component-gallery.md](2026-07-21-app-preview-component-gallery.md) | `:app-preview` 컴포넌트 갤러리 앱(10 Task: 카탈로그+NavKey17 → PreviewSection → Button6화면 → Input2 → Text3 → Container5 → Bar1(YGTopBar4변형) → EntryBuilders+DI배선 → MainScreen그룹목록+Route → 실기기검증). core:designsystem YG* 17종 실렌더 갤러리, 상태형은 remember 인터랙션, navigator.goTo/onBack. NavKey는 파일당 1개(NavKeyYG*.kt 17). 각 화면·MainScreen·PreviewSection 자체 @YGPreview. 기존 Navigation3+Hilt multibinding 확장. 테스트 없음(compile+assembleDebug+육안). 메인 상단은 Text 제목(루트 back 대상 없어 YGTopBarDetail 미채택). 스펙: [specs](../specs/2026-07-21-app-preview-component-gallery.md) |
