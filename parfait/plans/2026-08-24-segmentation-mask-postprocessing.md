...
배선은 주 경로(ML Kit 후보 판)와 폴백 경로(전경 신뢰도 마스크) 양쪽에 놓는다.
...
- **세 PR 모두 베이스는 `develop`이다.** PR #349 위에 스택하지 않는다 — 파일 교집합이 0이라 스택할
  근거가 없고, 스택하면 #349 리뷰가 늦어질 때 이쪽이 전부 막힌다.
...
- 테스트는 `kotlin.test`(`@Test`·`assertEquals`·`assertNull`·`assertTrue`·`assertContentEquals`·
  `assertFailsWith`)를 쓴다. `org.junit.*`을 import하지 않는다. 이름은 `함수명_조건_기대`이고
...
- **기존 파일을 "전문"으로 덮어쓰지 않는다.** 항상 추가·치환 지시로 고친다.
...
| 파일 | 책임 | 단계 |
|---|---|---|
| `data/.../repository/image/AlphaCoverage.kt` (신설) | 픽셀 배열의 알파 총합 (순수) | 1 |
...
| `data/.../repository/image/AlphaComponents.kt` (신설) | 축소·런·union-find·area opening·팽창 (순수) | 2 |
| `data/src/test/.../repository/image/AlphaComponentsTest.kt` (신설) | 위 함수들의 유닛 | 2 |
| `data/.../repository/image/AlphaPostProcessor.kt` (신설) | 적용·침식·측정·조립 (순수) | 2 |
...
**패키지는 전부 `com.teamyg.parfait.data.repository.image`다**(신설 파일 포함).

---

# 1단계 — 필터 판정 (PR 1)

`develop`에서 브랜치를 판다: `git checkout develop && git pull && git checkout -b feature/segmentation-candidate-coverage`
...
```bash
git add data/src/main/java/com/teamyg/parfait/data/repository/image/AlphaCoverage.kt \
        data/src/test/java/com/teamyg/parfait/data/repository/image/AlphaCoverageTest.kt
...
Run: `./gradlew :data:compileDebugKotlin :feature:segmentation:impl:compileDebugUnitTestKotlin`
Expected: FAIL — `No value passed for parameter 'coverageAlphaSum'`가 네 곳에서 난다
(`ImageSegmentationRepositoryImpl` 두 곳, `SegmentationCandidateFilterTest` 헬퍼,
`SegmentationViewModelTest` 두 자리).

- [ ] **Step 3: 주 경로 생성 지점을 채운다**

`ImageSegmentationRepositoryImpl.kt`의 `toCandidates` 안, `subjectBitmap`을 얻은 직후에 픽셀을
읽어 총합을 낸다. `SegmentationCandidate(...)` 호출에 `coverageAlphaSum = ...`을 더한다:
...
```kotlin
        subjects.mapNotNull { subject ->
            ...
            subjectBitmap.getPixels(
                pixels, 0, subjectBitmap.width, 0, 0, subjectBitmap.width, subjectBitmap.height,
            )
...
같은 파일 `toForegroundCandidate`의 `listOf(SegmentationCandidate(...))` 호출에 한 줄을 더한다.
그 함수에는 이미 `pixels`가 있지만 **`maskSubjectPixels`가 그 자리에서 투명으로 지운 뒤**이므로
그대로 쓰면 된다:
...
`SegmentationCandidateFilterTest.kt`의 `candidate(...)` 헬퍼에 파라미터를 더한다. **기본값은
"bounds를 꽉 채운 불투명 판"으로 둔다** — 기존 테스트가 사각형 면적을 전제로 쓰여 있으므로 그
전제를 그대로 옮겨 놓아야 Task 3에서 무엇이 바뀌는지 보인다:
...
private fun candidate(
    ...
    bottom: Int = 10,
    canvasWidth: Int = 100,
    canvasHeight: Int = 100,
    coverageAlphaSum: Long = 255L * (right - left) * (bottom - top),
) = SegmentationCandidate(
    bounds = SegmentationBounds(left = left, top = top, right = right, bottom = bottom),
    bitmap = FakeBitmap,
...
⚠️ 기존 헬퍼의 파라미터 이름·기본값이 위와 다르면 **기존 것을 유지하고 `coverageAlphaSum` 한 줄만
더한다.** 헬퍼 시그니처를 통째로 갈면 기존 호출부가 전부 깨진다.

- [ ] **Step 6: ViewModel 테스트의 두 자리를 채운다**

`SegmentationViewModelTest.kt`의 `SegmentationCandidate(...)` 두 곳에 `coverageAlphaSum = 255L * 100`을
더한다. 그 테스트는 `bounds`를 안 보므로 값은 판정을 통과하기만 하면 된다.
...
- Produces: `MIN_SUBJECT_COVERAGE_PERMYRIAD`, `MIN_SUBJECT_COVERAGE_PIXELS`, `coverageFloorPixels(canvasArea: Long): Long`

- [ ] **Step 1: 실패하는 테스트를 쓴다**

`SegmentationCandidateFilterTest.kt`에서 **면적 관련 기존 테스트 넷을 지우고**
(`areaIsExactlyTheThreshold`·`areaIsBelowTheThreshold`·`everyCandidateIsBelowTheThreshold`·
`sameArea_ordersByTopThenLeft`) 아래를 넣는다. `moreThanTheLimit_keepsTheBiggestOnes`는 정렬 키가
바뀌므로 함께 다시 쓴다:

```kotlin
    @Test
    fun filterCandidates_coverageIsExactlyTheFloor_keepsIt() {
        // Given — 캔버스 1,000,000px 이면 하한은 max(2500, 500) = 2500px = 알파 총합 637,500
        val exactly = candidate(canvasWidth = 1_000, canvasHeight = 1_000, coverageAlphaSum = 255L * 2_500)

        // When
        val kept = filterCandidates(listOf(exactly))

        // Then
        assertEquals(listOf(exactly), kept)
    }

    @Test
    fun filterCandidates_coverageIsBelowTheFloor_dropsIt() {
        // Given
        val below = candidate(canvasWidth = 1_000, canvasHeight = 1_000, coverageAlphaSum = 255L * 2_499)

        // When
        val kept = filterCandidates(listOf(below))
    ...
    fun filterCandidates_bigCanvas_ratioFloorTakesOverTheAbsoluteFloor() {
        ...
        val below = candidate(canvasWidth = 4_000, canvasHeight = 3_000, coverageAlphaSum = 255L * 5_999)
        val above = candidate(canvasWidth = 4_000, canvasHeight = 3_000, coverageAlphaSum = 255L * 6_000)

        // When
        val kept = filterCandidates(listOf(below, above))

        // Then
        assertEquals(listOf(above), kept)
    }
    ...
    fun filterCandidates_thinButLargeSubject_survivesWhileTinyFragmentDoesNot() {
        ...
        val pen = candidate(
            ...
            canvasWidth = 4_000, canvasHeight = 3_000, coverageAlphaSum = 255L * 10_000,
        )
        val fragment = candidate(
            left = 0, top = 0, right = 900, bottom = 900,
            canvasWidth = 4_000, canvasHeight = 3_000, coverageAlphaSum = 255L * 300,
        )

        // When
        val kept = filterCandidates(listOf(fragment, pen))

        // Then
        assertEquals(listOf(pen), kept)
    }
    ...
    fun filterCandidates_twelveMegapixelOpaqueSubject_survivesTheIntBoundary() {
        ...
        val fullFrame = candidate(
            left = 0, top = 0, right = 4_000, bottom = 3_000,
            canvasWidth = 4_000, canvasHeight = 3_000,
            coverageAlphaSum = 255L * 12_000_000,
        )

        // When
        val kept = filterCandidates(listOf(fullFrame))

        // Then
        assertEquals(listOf(fullFrame), kept)
    }

    @Test
    fun filterCandidates_sameCoverage_ordersByTopThenLeftThenBottomThenRight() {
        // Given — 커버리지가 같고 좌상단도 같은데 크기가 다른 쌍을 섞는다
        val taller = candidate(left = 1, top = 1, right = 40, bottom = 60, coverageAlphaSum = 255L * 5_000)
        val shorter = candidate(left = 1, top = 1, right = 40, bottom = 50, coverageAlphaSum = 255L * 5_000)
        val lower = candidate(left = 1, top = 9, right = 40, bottom = 60, coverageAlphaSum = 255L * 5_000)

        // When
        val kept = filterCandidates(listOf(lower, taller, shorter))

        // Then
        assertEquals(listOf(shorter, taller, lower), kept)
    }
```

⚠️ `sameCoverage_ordersBy...` 는 세 후보가 서로 IoU 0.9 미만이어야 병합에 안 걸린다.
`shorter`(39×49)와 `taller`(39×59)의 IoU는 `1911 / 2301 ≈ 0.83`이라 안전하다.
 ...
 *
 * 주 경로가 후처리 전에 값싼 사전 절단을 할 때도 이 값을 쓴다 — bbox 픽셀 수는 커버리지의
 * 상계이므로, bbox가 이 값 미만이면 커버리지도 미만이다.
 */
 ...
 * 없으면 동률에서 순서가 ML Kit 반환 순서를 따라가고, 탐욕 병합이 그 순서에 민감하다.
 */
...
`SegmentationCandidateFilterTest.kt`에 더한다:
...
```kotlin
    ...
    fun filterCandidates_iouAboveTheThreshold_keepsTheOneWithMoreCoverage() {
        ...
        val bigger = candidate(
            ...
            canvasWidth = 1_000, canvasHeight = 1_000, coverageAlphaSum = 255L * 9_000,
        )
        val nearlySame = candidate(
            left = 2, top = 2, right = 98, bottom = 98,
            canvasWidth = 1_000, canvasHeight = 1_000, coverageAlphaSum = 255L * 8_000,
        )

        // When
        val kept = filterCandidates(listOf(nearlySame, bigger))

        // Then
        assertEquals(listOf(bigger), kept)
    }
    ...
    fun filterCandidates_iouBelowTheThreshold_keepsBoth() {
        ...
        // When
        val kept = filterCandidates(listOf(overlapping, bigger))

        // Then
        assertEquals(listOf(bigger, overlapping), kept)
    }
    ...
    fun filterCandidates_smallSubjectInsideABigOne_keepsBoth() {
        ...
        val person = candidate(
            left = 0, top = 0, right = 300, bottom = 900,
            canvasWidth = 1_000, canvasHeight = 1_000, coverageAlphaSum = 255L * 100_000,
        )
        val heldItem = candidate(
            left = 100, top = 400, right = 180, bottom = 480,
            canvasWidth = 1_000, canvasHeight = 1_000, coverageAlphaSum = 255L * 5_000,
        )

        // When
        val kept = filterCandidates(listOf(heldItem, person))

        // Then
        assertEquals(listOf(person, heldItem), kept)
    }
```
...
`SegmentationCandidateFilter.kt`에 더하고 `filterCandidates`를 바꾼다:
 ...
 *
 * ⚠️ **포함 관계는 병합하지 않는다.** 사람 안의 든 물건은 IoU 가 낮아 살아남는데, 사용자가 어느
 * 피사체를 오릴지 고르는 화면이므로 그게 맞다. 교집합을 작은 쪽 면적으로 나누는 지표로 바꾸면
 * 든 물건이 지워진다. 근거는 `specs/2026-08-24-segmentation-mask-postprocessing.md` 「필터 판정」.
 */
...
Run: `./gradlew test ktlintCheck :app:assembleDebug`
Expected: PASS. 기존 `duplicateBounds_keepsOnlyOne`은 IoU 1이라 그대로 통과하고,
`emptyInput_returnsEmpty`도 그대로다.
...
`develop`에서 별도 브랜치를 판다: `git checkout develop && git checkout -b feature/segmentation-alpha-kernel`

이 단계는 **호출부를 만들지 않는다.** 3단계가 같은 스프린트에 붙는 것을 전제로 감수한다.
⚠️ **"아직 아무도 안 부른다"는 주석을 달지 않는다** — 3단계 머지 즉시 거짓이 되고, 그것은
`parfait/CLAUDE.md`가 금지하는 "다른 컴포넌트의 현재 상태" 서술이다.
...
**Interfaces:**
- Consumes: 없음
- Produces: `internal fun ceilDiv(value: Int, divisor: Int): Int`,
  `internal fun downscaleMask(alpha: ByteArray, width: Int, height: Int, factor: Int, threshold: Int): BooleanArray`
...
- Produces: `internal fun applyAreaOpening(mask: BooleanArray, width: Int, height: Int, minPixels: Int): Boolean`

- [ ] **Step 1: 실패하는 테스트를 쓴다**

`AlphaComponentsTest.kt`에 헬퍼와 테스트를 더한다:
...
private fun countRuns(mask: BooleanArray, width: Int, height: Int): Int {
    var count = 0
    for (y in 0 until height) {
        val rowOffset = y * width
        var x = 0
...
private fun findRoot(parent: IntArray, node: Int): Int {
    var root = node
...
- Produces: `internal fun dilateMask(mask: BooleanArray, width: Int, height: Int): BooleanArray`
 ...
 */
internal fun dilateMask(mask: BooleanArray, width: Int, height: Int): BooleanArray {
...
**Interfaces:**
- Consumes: `ceilDiv` (Task 5)
- Produces: `internal data class KeepMaskApplication(val alphaSum: Long, val changed: Boolean)`,
  `internal fun applyKeepMask(alpha, width, height, keep, maskWidth, factor, checkCancelled): KeepMaskApplication`,
  `internal data class AlphaMeasurement(val bounds: SegmentationBounds, val alphaSum: Long, val partialAlphaPixels: Int)`,
  `internal fun measureAlpha(alpha, width, height, checkCancelled): AlphaMeasurement?`
...
class AlphaPostProcessorTest {
    @Test
    fun applyKeepMask_maskIsFalse_clearsThatBlockOnly() {
        // Given — 2×2 원본, 배율 2 라 축소판은 1픽셀이다
        val alpha = alphaBytes(255, 255, 255, 255)
        val keep = booleanArrayOf(false)

        // When
        val applied = applyKeepMask(alpha, width = 2, height = 2, keep = keep, maskWidth = 1, factor = 2)

        // Then
        assertContentEquals(intArrayOf(0, 0, 0, 0), alpha.asInts())
        assertEquals(0L, applied.alphaSum)
        assertEquals(true, applied.changed)
    }
    ...
    fun applyKeepMask_maskIsTrue_leavesTheOriginalAlphaUntouched() {
        ...
        assertEquals(465L, applied.alphaSum)
        assertEquals(false, applied.changed)
    }
...
internal data class KeepMaskApplication(
    /** 침식 전 알파 총합. 후처리를 되돌릴 때 그 후보의 커버리지로 쓴다 */
    val alphaSum: Long,
    val changed: Boolean,
)
...
internal fun applyKeepMask(
    ...
    for (y in 0 until height) {
        ...
        for (x in 0 until width) {
            ...
            if (keep[maskRowOffset + x / factor]) {
                sum += value
            } else {
                alpha[index] = 0
                changed = true
    ...
    return KeepMaskApplication(alphaSum = sum, changed = changed)
}
 ...
 *
 * 불투명 판정을 "알파 0 초과"로 두는 것은 `ToppingEditMask.kt#trimTransparentBounds` 와 맞추기
 * 위해서다. 둘이 다른 답을 내면 편집·저장 경로의 판 크기가 후보 화면과 어긋난다.
 *
...
**Interfaces:**
- Consumes: 없음
- Produces: `internal fun erodeEdge(alpha: ByteArray, width: Int, height: Int, checkCancelled: () -> Unit = {}): Boolean`
...
**Interfaces:**
- Consumes: `downscaleMask`·`applyAreaOpening`·`dilateMask` (Task 5~7),
...
```kotlin
    ...
    fun postProcessAlpha_cancelledMidway_propagatesTheCallersThrow() {
        ...
        // 임계를 낮춰야 성분이 살아남아 콜백이 여러 번 불리는 구간까지 간다
    ...
    fun postProcessAlpha_sameSizeSpeckOnAndOffTheBlockGrid_isCountedDifferently() {
        ...
        val options = AlphaPostProcessOptions(
            downscaleFactor = 4, areaOpeningMinPixels = 32, erodeEdge = false, minPixelsForDownscale = 0,
        )
    ...
    fun postProcessAlpha_speckCloseToTheBlob_isNotRemovable() {
        ...
        alpha[0 * 32 + 17] = 255.toByte()

        // When
        val result = postProcessAlpha(
            alpha, width = 32, height = 32,
            options = AlphaPostProcessOptions(
                downscaleFactor = 4, areaOpeningMinPixels = 64, erodeEdge = false, minPixelsForDownscale = 0,
            ),
        )
    ...
    fun postProcessAlpha_downscaleFour_keepsTheBlobAndDropsTheDistantSpeck() {
        // Given — 배율 4 층위. 32×32 에 16×16 덩어리와 멀리 떨어진 4×4 점
        val alpha = ByteArray(32 * 32)
        for (y in 0 until 16) for (x in 0 until 16) alpha[y * 32 + x] = 255.toByte()
        ...
        val result = postProcessAlpha(
            alpha, width = 32, height = 32,
            options = AlphaPostProcessOptions(
                downscaleFactor = 4, areaOpeningMinPixels = 64, erodeEdge = false,
                minPixelsForDownscale = 0,
            ),
        )

        // Then — 팽창 1블록이 붙어 bounds 가 덩어리보다 넉넉하지만 점 쪽으로는 안 늘어난다
    ...
    @Test
    fun postProcessAlpha_smallCanvas_skipsTheDownscale() {
        ...
        val result = postProcessAlpha(
            alpha, width = 8, height = 8,
            options = AlphaPostProcessOptions(
                downscaleFactor = 4, areaOpeningMinPixels = 4, minPixelsForDownscale = 1_000,
            ),
        )
...
internal data class AlphaPostProcessResult(
    val bounds: SegmentationBounds,
    val alphaSum: Long,
    /** 침식 전 총합. 후처리를 되돌린 후보의 커버리지가 된다 */
    val alphaSumBeforeErode: Long,
    val partialAlphaPixels: Int,
    /** 거짓이면 호출부가 원본 판을 그대로 쓸 수 있다 */
    val changed: Boolean,
)
...
internal fun postProcessAlpha(
    ...
    return AlphaPostProcessResult(
        ...
        alphaSumBeforeErode = applied.alphaSum,
        partialAlphaPixels = measured.partialAlphaPixels,
        changed = applied.changed || eroded,
    )
}
```
...
**Interfaces:**
- Consumes: `postProcessAlpha` (Task 10)
- Produces: `internal fun confidenceToAlpha(confidence: Float): Int`,
  `internal class MaskedAlpha(val alpha: ByteArray, val result: AlphaPostProcessResult)`,
  `internal fun maskSubjectAlpha(mask: FloatBuffer, width: Int, height: Int, checkCancelled: () -> Unit = {}): MaskedAlpha?`

⚠️ `maskSubjectPixels`는 **삭제한다.** 픽셀 배열을 더 이상 받지 않으므로 이름이 거짓이 된다.

- [ ] **Step 1: 실패하는 테스트를 쓴다**

`SegmentationMaskTest.kt`를 아래로 재작성한다. 기존 6개 중 임계 테스트의 의도(초과·이상 구분)는
`confidenceToAlpha` 경계 4점으로 이관되고, 버퍼 limit 테스트는 그대로 유지된다:
...
private fun confidenceBuffer(vararg values: Float): FloatBuffer =
...
class SegmentationMaskTest {
    ...
    fun maskSubjectAlpha_confidentBlobWithASpeck_dropsTheSpeck() {
        ...
        // When
        val masked = maskSubjectAlpha(confidenceBuffer(*values), width = 8, height = 8)
    ...
    fun maskSubjectAlpha_bufferLimitBelowCapacity_stopsAtLimitInsteadOfReadingPastIt() {
        ...
        assertFailsWith<IndexOutOfBoundsException> {
            maskSubjectAlpha(buffer, width = 8, height = 8)
        }
    }
}
```
...
`SegmentationMask.kt`를 아래로 갈아 넣는다. `SUBJECT_CONFIDENCE_THRESHOLD`와
`maskSubjectPixels`는 삭제한다:
...
internal fun confidenceToAlpha(confidence: Float): Int {
    ...
    if (confidence >= RAMP_CEILING) return OPAQUE

    return (OPAQUE * (confidence - RAMP_FLOOR) / (RAMP_CEILING - RAMP_FLOOR)).toInt()
}
...
Run: `./gradlew :data:testDebugUnitTest --tests "*SegmentationMaskTest*"`
Expected: PASS. `ImageSegmentationRepositoryImpl`은 아직 `maskSubjectPixels`를 부르므로
`:data:compileDebugKotlin`은 실패한다 — Task 12가 고친다.

- [ ] **Step 5: 커밋하지 않는다**

이 태스크는 컴파일이 깨진 상태로 끝난다. Task 12와 함께 커밋한다.
...
**Interfaces:**
- Consumes: `maskSubjectAlpha`·`MaskedAlpha` (Task 11), `sumAlpha` (Task 1)
- Produces: 없음 (private)

**테스트가 없는 이유:** 이 함수는 `Bitmap.getPixels`·`Bitmap.createBitmap`을 부른다. 이 저장소에
Robolectric이 없고 `isReturnDefaultValues`도 꺼져 있어 JVM 유닛에서 터진다. 판단은 전부 Task 11의
순수 함수로 빠져 있고 여기 남은 것은 좌표 산술과 비트맵 생성뿐이다.

- [ ] **Step 1: `toForegroundCandidate`를 갈아 넣는다**

기존 본문(픽셀 배열 생성 → `maskSubjectPixels` → `masked` 비트맵 → `createBitmap` 으로 자르기 →
`recycle` 분기)을 아래로 치환한다:
...
```kotlin
    ...
    private fun SubjectSegmentationResult.toForegroundCandidate(
        ...
        val masked = maskSubjectAlpha(foregroundMask, width, height, checkCancelled) ?: return emptyList()
        ...
        origin.getPixels(trimmedPixels, 0, bounds.width, bounds.left, bounds.top, bounds.width, bounds.height)
        applyAlphaInPlace(trimmedPixels, masked.alpha, width, bounds)

        val trimmed = Bitmap.createBitmap(trimmedPixels, bounds.width, bounds.height, Bitmap.Config.ARGB_8888)
    ...
    private fun applyAlphaInPlace(
        ...
        for (y in 0 until bounds.height) {
            ...
            for (x in 0 until bounds.width) {
                ...
                pixels[pixelRow + x] = if (value == 0) 0 else (value shl 24) or (pixels[pixelRow + x] and 0x00FFFFFF)
            }
        }
    }
```

- [ ] **Step 2: `segmentForeground`가 콜백을 넘기게 고친다**

```kotlin
        return try {
            withContext(Dispatchers.Default) {
                val job = currentCoroutineContext()[Job]
                result.toForegroundCandidate(origin) { job?.ensureActive() }
            }
...
import를 더한다: `kotlinx.coroutines.Job`, `kotlinx.coroutines.currentCoroutineContext`,
`kotlinx.coroutines.ensureActive`, `com.teamyg.parfait.domain.model.SegmentationBounds`.
...
**Interfaces:**
- Consumes: `postProcessAlpha` (Task 10), `sumAlpha` (Task 1), `coverageFloorPixels` (Task 3)
- Produces: `private fun SubjectSegmentationResult.toCandidatePairs(origin, checkCancelled): List<CandidatePair>`,
  `private class CandidatePair(val original: SegmentationCandidate, val postProcessed: SegmentationCandidate?)`

**테스트가 없는 이유:** Task 12와 같다. 비트맵 API가 걸려 JVM 유닛으로 못 덮는다.

- [ ] **Step 1: 사전 절단과 후처리를 넣는다**

`toCandidates`를 아래로 치환한다:
...
```kotlin
    ...
    private fun SubjectSegmentationResult.toCandidatePairs(
        ...
        val canvasArea = origin.width.toLong() * origin.height
        val floor = coverageFloorPixels(canvasArea)
        ...
        return considered.map { (subject, bitmap) -> buildCandidatePair(subject, bitmap, origin, checkCancelled) }
    }
    ...
    private fun buildCandidatePair(
        ...
        val original = SegmentationCandidate(
            ...
            bounds = SegmentationBounds(
                left = subject.startX,
                top = subject.startY,
                right = subject.startX + width,
                bottom = subject.startY + height,
            ),
        ...
        val postProcessed = try {
            postProcess(subject, bitmap, pixels, origin, checkCancelled)
        ...
        return CandidatePair(original = original, postProcessed = postProcessed)
    }

    /**
     * ⚠️ **자르기는 알파를 바꾸지 않는다.** 후처리 결과를 픽셀에 반영하려면 새 판을 만들어야 한다.
     * ML Kit 판에 되쓰는 것은 안 된다 — 그 판의 수명은 `SubjectSegmentationResult` 가 쥐고 있고
     * 네이티브에서 온 비트맵이 immutable 이면 예외다(소유권 논의는 OQ-P-266).
     */
...
import를 더한다: `com.google.mlkit.vision.segmentation.subject.Subject`,
`com.teamyg.parfait.data.utils.repositoryLogger`.

- [ ] **Step 2: 빌드와 테스트를 돌린다**

Run: `./gradlew test ktlintCheck :app:assembleDebug`
Expected: FAIL — `segmentImage`가 아직 `toCandidates`를 부른다. Task 14가 고친다.
...
## Task 14: 전멸 원인별 분기와 되돌리기

**Files:**
- Modify: `data/src/main/java/com/teamyg/parfait/data/repository/image/ImageSegmentationRepositoryImpl.kt`

**Interfaces:**
- Consumes: `toCandidatePairs`·`CandidatePair` (Task 13), `filterCandidates` (Task 4)
- Produces: 없음

**테스트가 없는 이유:** `segmentImage`는 ML Kit 클라이언트와 `Bitmap`을 직접 잡는다. 분기 자체를
JVM으로 덮으려면 리포지토리를 갈라야 하는데 이 라운드의 범위를 넘는다. 세 갈래는 관측 로그가
필드에서 판정한다(Task 15).

- [ ] **Step 1: `segmentImage`의 후보 흐름을 갈아 넣는다**
...
```kotlin
        ...
        val candidates = try {
            withContext(Dispatchers.Default) {
                ...
                val pairs = result.toCandidatePairs(bitmap, checkCancelled)
                if (pairs.isEmpty()) return@withContext emptyList()

                val processed = pairs.mapNotNull { it.postProcessed }
                val effective = if (processed.isEmpty()) {
                    // 후처리는 개선 수단이지 후보를 없앨 권한이 아니다. 폴백으로 내려보내면
                    // ML Kit 을 한 번 더 돌리게 되는데, 그 비용은 "후보가 0건인 사진에서만
                    // 든다"는 전제로 정당화돼 있다
                    repositoryLogger.i { "세그멘테이션 후처리가 후보를 전멸시켜 원본 후보로 되돌린다" }
                    pairs.map { it.original }
                } else {
                    processed
                }

                repositoryLogger.i {
                    "세그멘테이션 후처리: 대상 ${pairs.size}개 중 ${processed.size}개 생존"
                }

                filterCandidates(effective)
            }
        } catch (e: CancellationException) {
        ...
        // 여기 오는 갈래가 둘이다 — ML Kit 이 subject 를 하나도 안 줬거나, 필터가 전부 걸러 냈거나.
        // 둘 다 종전대로 폴백을 탄다. "후처리가 전멸시킨" 세 번째 갈래는 위에서 되돌려 여기 안 온다
        repositoryLogger.i { "세그멘테이션 다중 후보가 0건이라 전경 마스크 폴백으로 내려간다" }
...
**Interfaces:**
- Consumes: `AlphaPostProcessResult.partialAlphaPixels` (Task 10)
- Produces: 없음

부분 알파 비율은 OQ-P-287(램프 띠 폭)과 OQ-P-288(침식 유효성) 두 미결의 전제를 동시에 판정한다.
비율이 0에 가까우면 침식 단계는 남길 이유가 없다.

- [ ] **Step 1: `postProcess`에 로그 한 줄을 더한다**

`result`를 얻은 직후, `inner`를 꺼내기 전에:

```kotlin
        repositoryLogger.i {
            val total = width * height
            "세그멘테이션 후보 부분 알파 ${result.partialAlphaPixels}/$total"
        }
```

- [ ] **Step 2: 폴백 경로에도 같은 줄을 더한다**

`toForegroundCandidate`에서 `masked`를 얻은 직후:
...
```bash
git add -A
git commit -m "feat: 부분 알파 비율을 남겨 램프와 침식을 판정할 근거를 만든다"
```

---

## Task 16: 탭 승자 역전 회귀 테스트
...
**Interfaces:**
- Consumes: `pickCandidateIndex`
- Produces: 없음

`pickCandidateIndex`는 겹친 후보 중 bbox 면적이 최소인 것을 고른다. 후처리 축소량은 후보마다
다르므로 축소 후 대소가 뒤집히면 **같은 자리를 탭했을 때 이전과 다른 후보가 잡힌다.** 규칙 자체는
유지하되 그 민감성을 테스트로 고정한다.

- [ ] **Step 1: 테스트를 더한다**

```kotlin
    @Test
    fun pickCandidateIndex_shrinkingOneBoundsFlipsTheWinner_isFixedByThisTest() {
        ...
        // When
        val before = pickCandidateIndex(listOf(beforeA, beforeB), x = 10f, y = 10f)
        val after = pickCandidateIndex(listOf(afterA, afterB), x = 10f, y = 10f)
    ...
    fun pickCandidateIndex_lowAlphaResidueWidensBounds_alsoFlipsTheWinner() {
        ...
        // When
        val tight = pickCandidateIndex(listOf(tightA, tightB), x = 10f, y = 10f)
        val widened = pickCandidateIndex(listOf(widenedA, tightB), x = 10f, y = 10f)
...
⚠️ `pickCandidateIndex`의 실제 시그니처가 위와 다르면 **기존 테스트가 쓰는 호출 형태를 그대로
따른다.** 파라미터 이름·타입을 지어내지 않는다.
...
```bash
git add -A
git commit -m "test: bounds 축소가 탭 승자를 뒤집는 경우를 고정한다"
```
...
- 사진 세트로 근거 등급 표의 조건부 항목 다섯을 판정한다(프레임 변에 걸친 피사체, 머리카락이 있는
  인물, 밝은 배경의 밝은 물체, 비스듬히 놓인 가늘고 긴 물체, 잡티가 많은 텍스처 배경,
  짧은 변 512 미만). 앞 라운드의 세트를 이어 쓴다.
- 판정 결과를 스펙의 근거 등급 표와 OQ-P-287·288·292·294에 반영한다. **철회 조건을 만족하면 그
  항목을 빼는 것이 정답이다.**
- 계획 상태를 `done`으로 바꾸고 `archive/`로 옮긴다.
