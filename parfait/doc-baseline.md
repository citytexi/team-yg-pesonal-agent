...
- **요약**: **문서가 "임시"라고 적어 둔 자리 셋이 결정으로 바뀌었다**(delta 2건).
  두 PR이 성격은 다른데 한 문장으로 묶인다 — **비워 둔 자리를 채운 것이 아니라, 비워 둔 채로
  버티던 이유가 사라진 것**이다. #315는 온보딩 약관의 실패 표현 둘을 확정했고 #318은 하루 전
  서버가 거짓으로 만든 앱 주석 일곱을 고쳤다. 머지 커밋 **둘 다 트리가 브랜치 팁과 같아**
  충돌 해소 편집이 0건이고, 브랜치 사실이 그대로 develop 사실이다.
  **#315**(`refactor/term-agree-scaffold-v2`): 이름은 스캐폴드 이관인데 **실은 실패 표현 라운드다.**
  가입 실패가 갈래 넷을 전부 열거해 놓고 `viewModelLogger`로만 흘리던 것이(각 자리에
  `TODO(에러 UX 미정)`) `TermAgreeError` **2갈래**(`NETWORK`·`UNKNOWN`) + 공통 토스트가 됐다 —
  A-002 `LoginError`·S-101 `GroupSettingError`와 같은 형태이고 **가장 좁은 사례**다.
  `SignUpException.RequiredPolicyNotAgreed`처럼 **로그에는 결함으로 남기는 갈래까지 `UNKNOWN`으로
  접은 것**이 새로 드러난 점이다(사용자가 할 수 있는 일이 "잠시 후 다시"로 같다).
  **더 중요한 것은 같은 화면의 다른 실패가 반대로 갔다는 것이다** — 약관 조회 실패는
  `TODO(공통 에러화면)`이 예고하던 공용 화면으로 **가지 않기로** 정해져 목록 자리에 남았다.
  가른 기준이 OQ-P-167 ①("공통 에러화면을 세울지")의 답이다: **재시도 동선이 화면 안에 있으면
  화면에 남기고, 없으면 토스트로 알린다.** 컨테이너는 엔트리의 `YGScaffold`를 걷고 Route가
  `YGScaffoldV2`를 쥐며 `isLoading`에 **조회와 가입을 함께** 넘긴다(`isLoading || isSigningUp`,
  두 플래그를 겹쳐 넘기는 첫 사례). **V1 잔여 3 → 2파일**(`groups/enter`·`groups/canvas`
  EntryBuilder), 이관 화면 16 → **17개**. OQ-P-204 ①이 묻던 "이관만 하는 라운드"의 첫 사례인데
  **결선을 데려왔다** — 이 화면은 서버 결선이 진작 끝나 채울 것이 밀려 있던 자리였다.
  **#318**(`feature/#317-sync-backend-api-260820`): 2026-08-20 서버 delta(409 마감 가드)를 앱이 받았다.
  **"서버가 막지 않는다"고 단정한 주석 일곱을 지우지 않고 고쳤다** — 근거는 [parfait/CLAUDE.md](CLAUDE.md)
  "기준 2와 3이 겹칠 때는 남긴다"이고, 새 문장은 단정 대신 `api/parfait.md`를 가리킨다(OQ-P-244 ①).
  화면 방어는 그대로 남는다 — 지난 캔버스의 편집 진입을 치우는 것은 **실패를 보여 주기 전에 길을
  없애는 일**이라 서버 가드와 목적이 다르다. 같은 PR이 `ServerErrorCode.Parfait.PARFAIT_ALREADY_CLOSED`를
  **소비처 0건인 채로** 신설하며 그 파일의 "쓰는 코드만 둔다" 규약에 **"처분이 이미 정해진 코드는
  미리 둔다"**는 예외를 함께 적었다(처분은 c106 스펙이 정했다 — 이 코드만 토스트 후 캔버스로 되감는다).
  브랜치 PR2의 파일과 **바이트까지 같아** 리베이스 시 충돌하지 않는다.
  ⚠️ **그 상수가 남긴 경고가 이 라운드의 발견이다** — 다섯 경로 전부 **권한 검사가 마감 검사보다
  앞이라** 마감된 캔버스라도 남의 토핑이거나 비멤버면 409가 아니라 403이 먼저 온다. 마감을 유일한
  실패로 두고 분기하면 그 경우를 놓친다. `http/`에도 409 재현 요청과 **전 그룹을 마감시키는**
  테스트 전용 회전 경고가 붙었다.
  미결 정리: **해소 3건**(OQ-P-244 ① · OQ-P-167 ③④) · **신설 1건**(OQ-P-249 — 가입 중 플래그 KDoc이
  없는 버튼을 가리킨다. 동작 영향 0이나 오버레이 조건이 좁아지는 순간 방어가 사라진다) ·
  **부분 진전 2건**(OQ-P-204 2파일 · OQ-P-167 ②는 오히려 악화 — "알 수 없는 오류" 문구를 든 화면이 셋이 됐다).
  테스트: 유닛 560 → **561건**, 파일 64개 그대로.
  ⚠️ **실기기·실서버 확인 없음** — 이번에 처음 실패 토스트가 붙은 화면인데 그 토스트를 본 적이 없다.
