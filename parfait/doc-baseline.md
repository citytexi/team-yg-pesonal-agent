...
- **커밋**: `cbb48cd8` (`Merge pull request #366 from mash-up-kr/bugfix/#365-font`)
- **요약**: **바이너리 넷이 바뀐 라운드 — 그래서 무엇이 바뀌었는지를 커밋 메시지 말고는 아무도
  말해 주지 않는다**(delta 1건).
...
- **미머지 추적 항목**: **넷**(두 회차 연속으로 안 건드려 그대로다). 전부 세그멘테이션 라운드이고,
  ...
  **이번 회차가 확인한 것 하나** — **바이너리만 바뀐 라운드는 이 감사 체계가 가장 약한 자리다.**
