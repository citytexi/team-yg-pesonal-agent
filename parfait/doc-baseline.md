...
- **커밋**: `c55e10bc` (`Merge pull request #376 from mash-up-kr/feature/#373-sync-backend-api-260825`)
- **요약**: **문서가 세 라운드째 "미머지"로 세어 온 넷이 develop이 아닌 곳에서 이미 배포됐다**
  (delta 3건).
...
- **검증일**: 2026-08-26 (52회차)
  ...
  📌 **또 낡아 있었다** — 50회차(`df3f4cbe`, #368)와 51회차(`cbb48cd8`, #366)가 기준선 해시와 이력
  표는 갱신하면서 이 줄만 건너뛰어 `2026-08-25 (49회차)`에 멈춰 있었다. 이번에 52로 맞췄다.
  같은 종류의 누락이 **두 번째**이므로, 절차의 4번(기준선 갱신)에 이 줄이 포함된다는 것을 여기
  적어 둔다.
...
- **미머지 추적 항목**: **넷 — 다만 이번 회차에 그 성격이 바뀌었다.** 여전히 `origin/develop`에는
  없지만, **넷 다 `origin/release/version-0.0.3-3`에는 머지돼 있고 그 브랜치가 태그 `0.0.3`으로
  나갔다.** 즉 "아직 아무 데도 안 들어간 브랜치"가 아니라 **"develop을 건너뛴 브랜치"**다
  → OQ-P-311. 다음 회차의 첫 결정은 이 넷을 develop으로 되돌릴지, 아니면 감사 대상을 바꿀지다.
  아래 표의 성격 열은 그대로 유효하다(무엇을 바꾸는 브랜치인지는 안 변했다).
  ...
  **이번 회차가 확인한 것 하나** — **"미머지"는 "안 나갔다"가 아니다.** 이 문서는 브랜치가
