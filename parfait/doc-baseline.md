...
- **커밋**: `a5e8a760` (`Merge pull request #349 from mash-up-kr/feature/segmentation-preprocessing`)
- **요약**: **촬영이 최대 품질로 들어오기 시작했고, 누운 사진이 세워졌다**(delta 1건).
...
- **미머지 추적 항목**: **넷**(네 회차 만에 다시 생겼다). 전부 세그멘테이션 라운드이고, **문서는
  이미 있는데 코드가 `origin`의 브랜치에만 있는** 상태다.
