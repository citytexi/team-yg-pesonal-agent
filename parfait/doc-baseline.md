...
- **커밋**: `37ea970b` (`Merge pull request #350 from mash-up-kr/feature/#345-permission-screen-inset`)
- **요약**: **세 주 전에 "사라졌다"고 적어 둔 결함이 다른 갈래에 살아 있었다**(delta 1건).
...
- **미머지 추적 항목**: **넷**(이번 회차가 건드리지 않아 그대로다). 전부 세그멘테이션 라운드이고,
  **문서는 이미 있는데 코드가 `origin`의 브랜치에만 있는** 상태다. 네 브랜치 모두 `origin`에 그대로
  살아 있는 것을 이번에 다시 확인했다.
  ...
  **이번 회차가 확인한 것 하나** — **스펙이 통째로 들어오지 않는 라운드가 있다.** 근거 등급으로
