...
- **`utils/clickable/` 프리미티브**(`component/*` 아님 — Modifier·Indication, 아직 develop 미머지): `@Composable Modifier.clickableYG`(중복 클릭 방지 leading-throttle, `TimeSource.Monotonic`) + 리플 종류별 공개 변형 `clickableYGDimRipple`·`clickableYGScaleRipple`·`clickableYGMergeRipple`. **표준 `Modifier.clickable` 위에 `remember`된 throttle 게이트를 얹는 구조**(Approach 2) — focus/하드웨어 키/hover/시맨틱을 `clickable`에서 그대로 확보(앱 전역 대체 목표, 접근성 회귀 방지). 커스텀 `Modifier.Node`는 없음. 리플 팩토리 `ygDimRipple`(dim, `createRippleModifierNode` 위임)·`ygScaleRipple`(press 축소, `DrawModifierNode`) 모두 `IndicationNodeFactory`, 다중은 `YGCompositeIndicationNodeFactory`로 합성. → [clickableyg-throttle](../specs/2026-07-12-clickableyg-throttle.md) · [ygripple](../specs/2026-07-13-ygripple.md) · [clickableyg-ripple-variants](../specs/2026-07-13-clickableyg-ripple-variants.md).
