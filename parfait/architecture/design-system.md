...
- **`utils/clickable/` 프리미티브**(`component/*` 아님 — Modifier·Indication, 아직 develop 미머지): `Modifier.clickableYG`(중복 클릭 방지 leading-throttle, 커스텀 `Modifier.Node`, `TimeSource.Monotonic`) + 리플 종류별 공개 변형 `clickableYGDimRipple`·`clickableYGScaleRipple`·`clickableYGMergeRipple`(코어 `internal clickableYGThrottle`이 `indications: List<Indication>` 수용, 노드가 다중 delegate). 리플 팩토리 `ygDimRipple`(dim ripple, `createRippleModifierNode` 위임)·`ygScaleRipple`(press 축소, `DrawModifierNode`) 모두 `IndicationNodeFactory`. `ClickableYGElement`는 수동 `equals`(onClick `!==`). → [clickableyg-throttle](../specs/2026-07-12-clickableyg-throttle.md) · [ygripple](../specs/2026-07-13-ygripple.md) · [clickableyg-ripple-variants](../specs/2026-07-13-clickableyg-ripple-variants.md).
