...
캔버스 응답의 `groupMembers`를 그리지만, 서버가 칩 색을 주지 않아 **목록 순서로 팔레트 7종을 돌려 쓴다**.
같은 그룹을 다시 열면 같은 사람에게 같은 색이 가는 것까지가 보장 범위이고 **사람에 고정된 것이 아니다**
(멤버가 빠지면 뒤가 밀린다). 글자는 닉네임 `take(1)` → [open-questions](../synthesis/open-questions.md).
