...
| 2026-08-19 | [parfait-doc-baseline](2026-08-19-parfait-doc-baseline.md) | 시리즈 2편. 구현 문서를 위키에서 분리한 이유(정본·수명 차이), `doc-baseline.md`·`api/server-baseline.md` 커밋 해시 기준선, delta만 감사하는 동기화 절차 2종, 주석 수명 규약, 기계 검사 부재 | 초안 |

## 시리즈 계획

1. **LLM 위키** (위 1편)
2. **문서-코드 기준선** (위 2편) — 구현 문서가 코드보다 빨리 낡는 문제.
   단방향 의존은 1편이 이미 한 절로 다뤄 2편에서는 분리 근거만 짧게 싣고 기준선 쪽으로 축을 옮겼다.
3. spec → plan → TDD 루프
