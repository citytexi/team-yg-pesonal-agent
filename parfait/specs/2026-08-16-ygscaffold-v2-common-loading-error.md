...
- **제출 중 전면 Dim을 어디까지 씌울지 미정.** S-002 계정 정보는 `isLoading`을 안 넘겼다 —
  실패가 입력 자리 인라인(`submitError`)이고 제출 중 표현도 확인 버튼 비활성이 이미 하고 있어서,
  그 위에 Dim을 얹으면 UX를 바꾸는 결정이 된다. 반면 S-003 앱 설정은 `isLoggingOut`을 넘겼다
  (끝나면 화면이 `replaceAll`로 사라지므로 그사이 조작이 의미가 없다). **"짧은 인라인 제출"과
  "화면을 떠나는 작업"의 경계가 문서화되지 않았고**, 지금은 화면별 판단이다.
