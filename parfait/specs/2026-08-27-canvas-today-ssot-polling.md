...
| 하루 경계 티커 발화 — 오늘 조회 | 같음 | 캐시 불변, 다음 주기가 재시도 |
| 폴링 주기 갱신 — 상세 조회 | 같음 | 캐시 불변, 조용히 다음 주기 |
| 쓰기 성공 뒤 강제 갱신 — 상세 조회 | 같음 | 캐시 불변, 다음 주기가 덮는다 |
| 폴링 시작(구독 0 → 1) 즉시 1회 | 위 규칙대로(캐시 유무·경계 여부에 따라) | 같음 |
...
> 없어 필터가 아예 돌지 않는다. 실제 보장은 PR3의 하루 경계 티커가 만든다. **PR2 단계에서는
> 기존과 같이 `Enter` 시점에만 오늘 판정이 갱신된다** — 이 절을 PR2의 검증 기준으로 쓰지 말 것.
...
**부르는 엔드포인트는 상황에 따라 갈린다.**

| 상황 | 부르는 것 |
|------|-----------|
| 캐시가 비었다(최초 획득) | 오늘 조회 `refreshTodayCanvas` |
| 하루 경계 티커가 발화했다 | 오늘 조회 `refreshTodayCanvas` |
...
필요가 있는 것은 최초 획득과 경계 전환뿐이므로 그 둘에만 남긴다.
...
티커를 두고, 그것을 두 곳이 함께 본다.

- **`CanvasPoller`** — 발화하면 다음 갱신을 오늘 조회로 전환한다(새 날 캔버스를 만들어야 하므로).
- **`CanvasMainViewModel`** — 발화하면 `today`·`selectedDate`·`displayedMonth`를 다시 센다.

값 스트림 하나에 매달지 않는 이유는 그것이 필요한 순간에 돌지 않기 때문이다. 캐시에
`distinctUntilChanged`가 걸려 있어 활동 없는 그룹에서는 재방출이 없고, 재방출이 없으면 필터도
평가되지 않는다. 필터가 도는 순간은 이미 캐시가 바뀐 뒤라 필터가 필요 없는 순간이다.

`CanvasMainViewModel`까지 함께 보게 하는 이유는 반대 방향의 구멍 때문이다. 지금 `syncToday()`를
부르는 곳은 `handleEnter()` 하나뿐이라, 화면을 열어 둔 채 경계를 넘기면 `today`·`selectedDate`가
어제로 남는다. 그러면 폴링이 받아 온 **오늘 캔버스가 어제 날짜 헤더 아래 그려진다.** 티커 하나로
두 방향이 같이 닫힌다.
...
- 캐시가 있고 경계가 아니면 상세 조회를, 캐시가 비었거나 경계 직후면 오늘 조회를 부른다
...
`parfaitId` / 캔버스 없음)도 단언한다. 하루 경계는 티커를 가상 시간으로 밀어 오늘 판정과 엔드포인트
전환이 함께 도는 것을 확인한다.
...
| `data/source/parfait/local/CanvasPoller.kt` | 그룹별 참조 계수 + 주기 갱신 루프 |
