...
`put`이 이전 값을 덮어쓰는 것 말고 비우는 자리를 두지 않는다. 그래서 홀더는 **언제나 최대 한 장**을
들고, 그 한 장은 다음 캡처까지 산다.
...
대가는 마지막 캡처 한 장(전체 해상도 ARGB_8888)이 다음 캡처까지 메모리에 남는 것이다. 캡처 PNG 한
장이 캐시에 남는 기존 결정(OQ-P-365 ①)과 성격이 같아 새 종류의 부담은 아니다.
