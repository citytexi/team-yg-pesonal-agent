...
| S3 | 커밋 `B`로 전환, 캐시에 `A`만 | `T_local` |
| S4 | 커밋 `B`, 캐시에 `A`와 `B` 모두, 출력은 삭제 | `T_remote` |
...
- **데몬 온도를 기록한다.** 데몬은 빌드를 거듭하며 JIT와 파일 해시를 축적해서 같은 데몬 안의
  ...
  `builds.csv`에 데몬 식별자와 그 데몬 안에서의 빌드 순번을 남기고, 회차마다 데몬을 새로
  띄운 뒤 고정 횟수로 워밍업한다. Kotlin 데몬도 별도 프로세스로 자체 웜업 곡선을 가지므로
  같은 취급을 받는다.
- **`--offline`으로 고정한다.** 의존성 캐시를 유지해도 플러그인 마커 재확인이나 툴체인 해석에서
  네트워크가 낄 수 있다.
...
- `builds.csv` — 빌드 1행.
- `tasks.csv` — 태스크 1행. 미스 사유 컬럼을 포함한다.
