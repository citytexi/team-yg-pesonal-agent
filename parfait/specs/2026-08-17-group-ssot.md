...
> 재조회**하게 만들고, 목록의 조회 실패 규칙도 바꿨다. 아래 "화면"과 "갱신 시점"은 그 결과를
> 전제한다. #288이 리뷰 중 바뀌면 이 스펙의 해당 대목도 따라 바뀐다.
...
- 재조회가 나가지 않는 경로에서도 목록이 맞다 — 나가기·신고가 캐시에서 그룹을 지우면 목록이
  그 자리에서 반영한다.
- 조회 실패 규칙은 **#288이 정한 것을 승계한다**: 보여 줄 목록이 있으면 남기고, 없을 때만 에러
  화면으로 넘긴다. `groupList`가 nullable이 되므로 판정은 `isNullOrEmpty()`가 된다.
