...
platforms: android
verified: 2026-08-13
related_code: LoginRoute, LoginScreen, LoginViewModel, KakaoLoginHelper, KakaoLoginResult, KakaoLoginVO, KakaoLoginResponse, AuthRepository, AuthRemoteDataSource, LoginWithKakaoUseCase, NonceGenerator, TokenStore, EncryptedTokenStore, NavKeyLogin, NavKeyTermAgree, NavKeyGroupList, TermAgreeRoute, RepositoryModule
...
//   invoke(idToken, nonce) = repository.loginWithKakao(...)
//       .onSuccess { if (it is KakaoLoginVO.ExistingMember) repository.saveSession(it.session) }
```
...
어느 경로든 종료 시 `isLoading = false`로 되돌린다.
...
- ⚠️ **개발 서버가 평문 HTTP면 요청이 차단된다**(기존 미결). 계획 첫 단계에 연결 확인을 넣는다.
...
- **약관 화면이 토큰을 받아 쓰지 않는 과도기**가 이 라운드의 산출물이다. signup 라운드까지
  `registrationToken`은 전달만 되고 소비되지 않는다.
