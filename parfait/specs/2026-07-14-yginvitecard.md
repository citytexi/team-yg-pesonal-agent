...
## 범위
- 포함: 라벨("그룹 초대 코드") + subText + 초대 코드 박스 + 복사 버튼(`YGButton` SmallSquare 재사용) 배치, Active/Invalid 상태별 토큰 분기, Active/Invalid 프리뷰.
- 제외:
...
- "그룹 초대 코드" 라벨은 컴포넌트 본질이라 내부 하드코딩 리터럴.

## 동작 / 상태
- 상태는 런타임 파생이 아니라 **prop(`status`)** 으로 주입. 컴포넌트는 stateless presentational.
- 복사 버튼: `YGButton(text = "복사", buttonType = YGButtonType.SmallSquare, isEnabled = status == Active, onClick = onCopyClick, endIconResource = ic_copy)`.
...
| 라벨 텍스트 | "그룹 초대 코드", `YGTheme.typography.body.b02R`, `YGAtomicColors.Gray.Gray400` |
...
| 코드 박스 padding | 세로 `YGTheme.layout.gap.gap3`, 가로 24dp |
...
- 코드 박스 가로 padding 24dp는 Figma literal(토큰 아님). 토큰화 여부는 SmallSquare 색 정비 브랜치의 디자인 토큰 정리와 함께 재확인.
