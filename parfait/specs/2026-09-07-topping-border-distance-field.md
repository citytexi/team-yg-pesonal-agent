...
| 2 | 띠 비트맵 | 거리판 + 양자화한 표시 긴 변 + 굵기px + (폴백 시 색) | 컴포저블 `remember` |

1단은 기존 마스크 캐시의 성질을 그대로 물려받는다 — 같은 `ImageRequest` 한 번의 결과로 만들고,
같은 모델의 동시 요청을 합류시키고, 로드를 컴포지션 밖 스코프에서 돌린다. 디코딩 횟수가 늘지
않는다.

2단은 캐시가 아니라 컴포지션 수명이라 화면에서 사라지면 함께 사라진다. 표시 긴 변을 격자로
반올림해 키에 넣으므로 핀치 중에도 격자를 넘을 때만 다시 만든다.
...
- **얇은 막대 회귀** — 폭 2px 막대에 outset 10을 주면 띠가 끊기지 않고 이어져야 한다.
  **여덟 방향 스탬프가 실패하는 바로 그 케이스**라, 이 테스트가 이번 작업의 목적을 코드로 고정한다.
...
5. 배치 화면 핀치 중 테두리가 따라오는가. 양자화 격자가 계단으로 보이는가.
6. 이미지 로드 실패 후 재시도했을 때 테두리와 판정이 함께 돌아오는가.

## 주의 / 열린 질문

- **`ALPHA_8` + tint 동작 미확인.** 계획 첫 태스크에서 검증한다. 실패 시 `ARGB_8888` 폴백.
- **양자화 격자 크기와 거리판 긴 변이 미측정이다.** 격자는 32px, 긴 변은 마스크가 쓰던 256을
  초기값으로 잡는다. 256은 판정 정밀도를 기준으로 고른 값이고 테두리의 요구는 다르므로, 육안
  확인 4·5번에서 재검토한다. 마스크 쪽 256이 미측정이라는 지적은 이미 열려 있다(OQ-P-313~316).
