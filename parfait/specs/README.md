...
| [2026-07-20-s004-terms-privacy-webview.md](2026-07-20-s004-terms-privacy-webview.md) | draft | `:feature:app:setting:impl` — S-004 약관/개인정보 화면 분리. EntryBuilder 두 NavKey를 ServiceTermsRoute/PrivacyPolicyRoute로 분리, 각 화면 = YGTopBarDetail(title 고정) + Notion 공개페이지 WebView. url은 ViewModel State 기본값(placeholder, 추후 UseCase 주입). NotionWebView 로컬 컴포넌트(로딩/에러 폴백, clipToBounds·onRelease·update tag 가드·LocalInspectionMode 프리뷰). 새 라이브러리 0개. 테스트 제외(프로젝트 미적용) |
| [2026-07-21-app-preview-component-gallery.md](2026-07-21-app-preview-component-gallery.md) | draft | `:app-preview` — core:designsystem YG* 컴포넌트 갤러리 앱. 메인=카테고리 그룹(Button/Input/Text/Container/Bar) 버튼 목록 → 탭 시 컴포넌트별 showcase 화면(변형·상태 실렌더, 상태 보유 컴포넌트는 remember 인터랙션). 카탈로그 데이터모델 + 컴포넌트별 NavKey 17개(파일당 1개)/EntryBuilder/@IntoSet DI + PreviewSection 공용 헬퍼. 각 화면·MainScreen·PreviewSection에 자체 @YGPreview 프리뷰 부착. 기존 Navigation3+Hilt multibinding 패턴 확장. feature 로컬 컴포넌트·designsystem 기존 @YGPreview 재사용·테스트 제외 |
| [2026-07-21-feature-common-terms-module.md](2026-07-21-feature-common-terms-module.md) | draft | `:feature:common:terms:{api,impl}` 신규 — S-004 약관/개인정보 화면(NavKey 2 + Route/Screen/VM 2세트 + NotionWebView + EntryBuilder)을 `:feature:app:setting`에서 이동. A-003 재사용 위한 feature/common 공유 레이어(ADR-0015). setting은 common:terms:api NavKey만 참조(goTo 유지), 자기 EntryBuilder에서 두 entry 제거. 동작 무변경 순수 이동. 문자열 title/에러는 common 신규(중복 허용) |
