...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `fa46e5cf`(2026-09-01 검증, #430까지 — **한 줄짜리 색 교정 하나가 들어왔고, 그것을 적어 둔 문서는 아카이브에 있었다**. delta 1건(#430 그룹 추가 팝업 배경색), **1파일 1/1**로 세어 온 회차 중 가장 작다. 트리 = 브랜치 팁(충돌 해소 편집 0건), 유닛 **1012**·계측 **17** 유지. 선작성 스펙·계획이 없어 **아카이브 이동 0건·신규 미결 0건**. G-001 그룹 추가 오버레이의 메뉴 판 배경이 `Cherry50` → `Gray.White` 로 바뀌었고 항목·구분선·모서리·계약은 그대로다. 유일한 드리프트가 아카이브에 들어간 g001 스펙 한 줄이라 **아카이브도 여전히 as-built 대조 대상**임이 드러났다. 직전 회차(`6a1da1b0`, #425까지) 요약은 doc-baseline 본문에 있다).
