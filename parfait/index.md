...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `69bbbe68`(2026-09-10 검증, #483까지 — **버전만 오른 회차인데, 새로 알아낸 것은 버전이 아니라 계보다**. delta 1건, **1파일 2/2**, `.kt` **0건**, 머지 트리 = 브랜치 팁. 유닛 **1229**·계측 **39** 그대로, 아카이브 이동 **0건**, 미결 **신설 0건**(`oq-next` 399 유지). `appVersionCode` 8 → **9** · `appVersionName` 1.1.1 → **1.1.2**(프리뷰 두 값 불변). `origin/release/version-1.1.2-9` 가 `origin/develop` HEAD **그 커밋**이고(양방향 0커밋) 직전 `1.1.1-8` 도 그랬다 — **OQ-P-311 ①(검증한 트리 ≠ 배포된 트리)이 최신 계보에서는 성립하지 않는다.** 지금 관행은 develop 에서 릴리즈 브랜치를 떼고 **버전 커밋을 develop 이 PR 로 되받는** 것이라 delta 가 곧 릴리즈 브랜치의 내용이다. ⚠️ 그 관행은 규칙으로 안 적혔고 `feature/debug-mode` 는 여전히 release 쪽에만 있다(②). 경량 태그 `1.1.2` 는 **경량인 채로 가리키는 곳만 develop 커밋이 됐다**(OQ-P-310 ② 불변). **버전 축을 지키는 테스트는 0건** — `DeviceInfoTest` 의 `1.1.1` 은 인자로 직접 넣는 픽스처라 카탈로그와 무관하다. `api/` 는 원격 연동 코드 0건이라 불변. 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`efa77150`, #482까지) 요약은 doc-baseline 본문에 있다).
