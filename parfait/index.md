...
서버 계약은 `api/`에 스냅샷돼 있다(도메인 8건·**엔드포인트 30개 + 테스트 전용 1**, 서버 `d76b27a`).
...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `efa77150`(2026-09-10 검증, #482까지 — **선작성 문서가 하루 만에 넉 벌 들어왔고, 그중 둘이 같은 자리를 순서대로 밟았다**. delta 5건, **78파일 2573/305**, 머지 다섯 전부 트리 = 브랜치 팁. 유닛 1164 → **1229**, 계측 37 → **39**. **선작성 스펙 4·계획 4 아카이브 이동** + **사후 스펙 2건 신규**(past-canvas-alert · canvas-feedback-fixes), 미결 **신설 6건**(OQ-P-392~397, `oq-next` 392 → 398)·**해소 1건**(OQ-P-389). #477 은 새 상태 없이 `lastClosedDate` 의 변화만 보고 알럿을 띄우되 **처음 확인은 기준선만 세우고** `markSeen` 은 띄우기로 확정된 뒤에만 부른다(⚠️ 정책 소스 없음, OQ-P-392). #478 은 앱 최초의 Analytics 소비처 — `simpleName` 을 안 쓰는 이유는 R8 이 **운영 집계에서만** 이름을 뭉개서이고, 판정 기준은 최상단 키가 아니라 **크기와의 짝**이다. #479 는 ViewModel 넷의 Repository 직접 주입을 UseCase 다섯으로 갈랐다(**위임이라는 사실 자체가 결정**). #480 은 축소 기준을 원본 사진으로 옮기고 **하한을 입력이 아니라 결과에** 건다. #482 는 적응형 폴링에 화면 수정 다섯이 얹혀 와, 로딩 덮개 최소 노출이 **`YGScaffoldV2` 의 계약**이 되고 첫 페인트 뒤 덮개 억제 판정이 두 번 무너졌다가 잡혔다(⚠️ 그 판정에 자기 테스트가 없다, OQ-P-395). **이 회차의 발견은 절차 쪽** — OQ-P-389 가 고쳐지자마자 같은 모양으로 재발했다(OQ-P-396). `api/` 는 원격 연동 코드 0건이라 계약 절 불변. 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`acbc4b45`, #473까지) 요약은 doc-baseline 본문에 있다).
