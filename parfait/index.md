...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `5907e286`(2026-09-06 검증, #458까지 — **선작성 스펙이 예고한 화면 하나가 들어왔고, 푸시 딥링크가 엣지 케이스 둘을 닫으며 버전이 1.1.0(코드 7)이 됐다**. delta 4건, **24파일 399/130**. 유닛 1091 → **1096**, 계측 **35** 유지, **선작성 스펙 1·계획 1 아카이브 이동**(대조 결과 설계와 코드가 어긋난 자리 0건), 미결 1건 해소(OQ-P-153)·2건 신설(374·375)·5건 정정(344·351·352·359·360). **이 회차의 성격은 앞 회차가 남긴 숙제를 걷는 일**이다 — 미머지 표기 셋, 개명된 심볼 하나, 앞 회차가 번호까지 적어 둔 낡은 문구 셋("등록 호출부 0건")을 걷었다. 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`489b14cc`, #450까지) 요약은 doc-baseline 본문에 있다).
