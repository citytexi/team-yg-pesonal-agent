...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `0173e454`(2026-09-01 검증, #434까지 — **위키가 적어 둔 목적지로 코드가 옮겨 왔고, 앱 버전이 1.0.0 이 됐다**. delta 5건(#412 핸들 모서리·#413 캔버스 상단바·#414 지난 캔버스 메뉴·#411 환영 배너·#434 버전), 25파일 449/85. 유닛 1012 → **1015**(+3)·계측 **17** 유지. 선작성 스펙·계획이 없어 **아카이브 이동 0건**, 미결 1건 해소(OQ-P-135)·2건 신설(OQ-P-339·340). 그룹 생성·참여의 종착지가 G-001 목록에서 **C-001 캔버스**로 옮겨 [[기능정의서-v6]] 배선과 맞았고, 복귀 관용구가 `replaceAll(목록)` + `goTo(캔버스)` 두 줄이 되며 `goToSingleClearTop` 소비처가 0건이 됐다. 갤러리 저장은 하단 메뉴에서 **날짜바 아이콘**으로 올라가 오늘 캔버스에서도 가능해졌다. **정본과 맞추는 변경이 정본에 없는 것(배너 문구·저장 노출 조건)을 함께 들여왔다**는 것이 이 회차의 형태다. 직전 회차(`fa46e5cf`, #430까지) 요약은 doc-baseline 본문에 있다).
