...
서버 계약은 `api/`에 스냅샷돼 있다(도메인 7건·**엔드포인트 28개 + 테스트 전용 1**, 서버 `a404ac2`).
...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `34bf1939`(2026-08-24 검증, #342까지 — **사진에 피사체가 여럿이면 이제 고를 수 있다, 그리고 실패가 다시 화면이 됐다**. delta 1건이고 머지 트리 = 브랜치 팁이라 충돌 해소 편집 0건. **선작성 스펙·플랜이 있는 첫 라운드가 여섯 회차 만에 돌아왔고 스택 둘이 한 PR로 합쳐져** 들어와 아카이브 이동이 스펙 1·플랜 2다. 정책에 처음부터 있던 C-103-select 갈래를 열었고, 후보를 하나로 접던 자리는 **ML Kit 옵션 한 곳**이었다. **화면 ID는 쪼개지 않았다** — `NavKeySegmentation` 하나가 후보 수에 따라 박스를 1개 또는 N개 그린다. 설계 핵심 셋: **탐지와 저장이 갈렸다**(`segmentImage`는 디스크를 안 건드리고 `persistSubject`가 탭한 하나만 저장, `SegmentationResult`는 `subjectBounds`를 잃고 좌표는 새 모델 `SegmentationCandidate`가 나른다) · **판단이 드는 자리를 순수 함수로 뺐다**(필터·좌표 변환·탭 판정, JVM 유닛 15건 — 이 화면에는 UI 테스트가 0건이다. 탭은 **면적 최소**를 골라 컴포넌트가 필터 정렬에 매달리지 않게 한다) · **선택 시점의 순서가 계약이다**(저장 → 초안 기록 완료 → 로딩 해제 → 이동). ⚠️ **두 옵션을 함께 켜면 ML Kit가 `SIGSEGV`로 죽는다**(OQ-P-268 해소 — `try/catch`도 못 잡고 `logcat -b crash`에만 남아, 전경 마스크는 후보 0건일 때의 2차 요청으로 남겼다). ✅ **실기기 확인이 0회가 아닌 드문 라운드**이고, 계획 밖에서 들어온 실패 화면이 PR #311이 삭제했던 `SegmentationErrorScreen`을 되살려 그 판정이 절반 뒤집혔다(OQ-P-153 ①②③ 해소, **로딩 화면 0개는 유지**). ⚠️ 남은 넷은 전부 실기기 항목이다 — OQ-P-266(후보 비트맵 미해제)·267(필터 상수 실측 없음)·269(`close()` 이후 수명, ①만 부분 확인)·277(같은 후보 재선택 시 초안 덮임). `api/`는 원격 연동 코드가 delta에 없어 미변경. 유닛 751 → **775건**, 클래스 87 → **89개**. 직전 기준선 `d634efd3`은 #336 — **편집 결과가 처음으로 남은** 라운드다(OQ-P-275·276·271). 이월된 위험은 그대로다 — **실서버 확인 0회**(OQ-P-146)·**다운샘플 상한 미측정**(OQ-P-228)·**Spotlight 라운드 테스트 0건**(OQ-P-252)).
