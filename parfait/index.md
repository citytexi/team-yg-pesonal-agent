...
서버 계약은 `api/`에 스냅샷돼 있다(도메인 8건·**엔드포인트 30개 + 테스트 전용 1**, 서버 `0c59af9`).
...
(PR #325, ADR-0013 철회 정정) 이제 보낼 자리가 생겼다. 다만 서버도 발송 인프라·알림 트리거를 범위
밖으로 명시해 무엇을 언제 보내는지는 아직 계약에 없다(OQ-P-341·OQ-P-343).
...
| Crashlytics·Analytics·Firebase 설정 (**푸시(FCM)는 2026-08-22 철회** — 되살릴 때 참고할 결정만 남아 있다. ⚠️ **철회 근거였던 "보낼 서버가 없다"가 2026-09-02 서버 delta로 사라졌다** — OQ-P-341) | [ADR-0013](adr/0013-firebase-fcm-crashlytics.md) |
...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `c74f40eb`(2026-09-03 검증, #444까지 — **앱을 여는 첫 그림이 갈렸는데 코드는 한 줄도 안 움직였다**. delta 1건(#444 스플래시 로띠 교체), **1파일 0/0**(바이너리). 유닛 **1029**·계측 **17** 유지, 아카이브 이동 0건, 미결 1건 신설(OQ-P-350)·2건 갱신(229·341). dotLottie 를 풀어 대조하니 manifest 판본·애니메이션 id·프레임률·재생 길이·레이어 구성이 교체 전과 같고 패스·키프레임만 달라 `R.raw.splash`·`SplashScreen` 무변경, 위키 [[스플래시-애니메이션]] A-001 정본의 60fps·3.5초 타임라인과 진입 대기 길이도 그대로다 → **드리프트 0건**. ⚠️ 바뀐 그림을 본 기록은 0건이고 옛 판으로 되돌아가는 것을 잡을 수단도 없다. **바이너리 교체라고 다 같은 것이 아니다** — 폰트와 달리 로띠는 열어서 구조를 대조할 수 있고, 이번 "드리프트 0건"의 근거는 커밋 메시지가 아니라 그 대조다. 미머지 브랜치는 둘에서 **다섯**으로 늘었다(캔버스 저장 미리보기·FCM 수신부·푸시 딥링크 신규). 직전 회차(`40e1fca6`, #442까지) 요약은 doc-baseline 본문에 있다).
