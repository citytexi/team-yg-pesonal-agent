...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `627e1867`(2026-08-28 검증, #397까지 — **버그를 고치는 김에 미결이 닫혔다**. delta 5건(#393·#394 그룹 생성·참여 수정 · #396 목록 토핑 Fit · #395 시스템바 · #397 토핑 회전·크기조절), 28파일 706/418, **다섯 머지 모두 트리 = 브랜치 팁**(충돌 해소 편집 0건). 선작성 스펙·플랜이 없어 **아카이브 이동 0건**, 유닛 931 → **942**(+11, 파일 98 유지)·계측 **14** 유지. **그룹 진입 두 화면이 한 관용구로 수렴했다** — 요청 직전에 확인 팝업을 닫고 로딩 오버레이 + 토스트로 알린다. `isEnabledButton` 호출자가 **0곳**이 되고(OQ-P-137 ④ 해소), V1 `YGScaffold` 잔여가 **1파일 1호출**로 줄었으며(OQ-P-204), 실패를 로그로만 남기던 A-005 가 `GroupCreateError` 2종을 얻어 OQ-P-167 ④ 가 그룹 진입에서 비었다. **읽기 전용이던 A-005 닉네임 필드가 열려** NavKey 인자가 표시값에서 초기값이 됐다(OQ-P-253 근거 흔들림). **목록 토핑이 `Crop` → `Fit`** 으로 바뀌어 누끼가 안 잘린다 — OQ-P-316 ②가 선행이라던 ① 없이 먼저 일어났다. **시스템바 라이트 고정**은 [ADR-0028](adr/0028-system-bar-light-fixed.md) 대로 들어와, 문서가 먼저 있고 코드가 따라온 유일한 사례다. **토핑 회전·크기조절이 상수에서 기하로** 옮겨 감도 상수 둘이 사라지고 환산이 화면으로 올라갔다(OQ-P-241 부분 해소, 넷 → 둘). ⚠️ S-102 의 닉네임 미적용 안내가 **이동을 지연**시키고 대기 상수가 `YGToastPolicy` 와 갈렸다 — 신규 미결 1건(OQ-P-328). 직전 회차(`84a89728`, #400까지) 요약은 doc-baseline 본문에 있다).
