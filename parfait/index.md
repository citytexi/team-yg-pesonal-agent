...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `5dc9fbec`(2026-08-27 검증, #389까지 — **한 스펙이 다섯 갈래로 나뉘어 들어왔고, 그 사이 스택의 머지 순서가 계획과 뒤집혔다**. delta 3건(#388·#390·#389)·17파일 1,208/129, 세 머지 다 트리 = 브랜치 팁이라 충돌 해소 편집 0건, 유닛 789 → **819**(+30)·계측 14 유지·아카이브 이동 2건(스펙·계획). **#390이 develop이 아니라 #389 브랜치로 머지**돼 develop 첫 부모 선에 붙은 것은 둘이다 — `--merges` 목록만 보면 안 드러난다. 토핑마다 걸린 클릭과 딤 클릭을 걷고 **레이어 전면 포인터 입력 하나**가 알파 마스크로 겹침 순서를 판정하며, "레이어가 캔버스 포인터를 독점한다"가 새 계약이 됐다. **계획의 "실행 뒤 달라진 것" 표가 리뷰 반영을 이미 담고 있어 드리프트는 셋**(상수 개명·`hitTestEnabled` 파라미터·배경 편집 마스크 좁히기). 배경 편집이 `YGToppingCutoutImage`로 갈아타 **OQ-P-254 해소**, OQ-P-203 ②는 대상 소멸, 신규 미결 5건(OQ-P-313~317). 미머지 넷 그대로이고 release 브랜치와의 격차는 **43커밋** 유지. 직전 회차(`bf06c830`, #371까지) 요약은 doc-baseline 본문에 있다).
