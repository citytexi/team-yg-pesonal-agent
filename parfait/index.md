...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `96dc215c`(2026-08-22 검증, #339까지 — **앱이 처음으로 화면 방향을 정했고 그 결정에는 시한이 붙어 있다**. `.kt` 0건인 첫 라운드로 매니페스트·문자열·버전 카탈로그·wrapper만 바뀌었다. `MainActivity`와 **카카오 리다이렉트 액티비티**까지 `screenOrientation="portrait"`, `<application>`에 대화면 opt-out 속성([ADR-0027](adr/0027-portrait-orientation-lock.md) 신설) — 리다이렉트를 빼면 로그인 구간에서만 회전이 살아 있고 그 구간이 A-002 로그인 유실 경로였다(OQ-P-146 ⑨ 재현 경로 닫힘). 앱 표시명 대문자화·`appVersionName` 하향·의존성 일괄 상향(코드 수정 0건인 채 CI `lint`·`unit-test` 통과)도 같은 머지다. ⚠️ **시한부다** — 대화면 opt-out은 `targetSdk 37`에 사라지는데 대화면 방침이 없고 추적처가 매니페스트 TODO 한 줄뿐이다(OQ-P-264). ⚠️ 세로 고정이 **카메라 촬영의 기준까지 고정**해 가로로 든 촬영 결과가 바뀌었다(OQ-P-265). 선작성 스펙·플랜이 없어 아카이브 이동 0건이고 계약 문서는 무변경, 유닛 737건·클래스 87개 유지. 직전 기준선 `8eb2af7d`는 #329 — **고른 배경이 처음으로 남고 편집 화면이 mock을 버린** 라운드다(C-301 확인이 색 PATCH / 업로드 후 `imageId` PATCH / 요청 0건 세 갈래, `api/parfait.md` `done`, 토핑 탭이 서버 캔버스를 그린다. ⚠️ 소유 판정이 축이 다른 두 id를 견주고 그것이 편집 게이트다 — OQ-P-250). 이월된 위험은 그대로다 — **실기기·실서버 확인 0회**(OQ-P-146, 쓰기 경로가 둘이다)·**다운샘플 상한 미측정**(OQ-P-228)·**Spotlight 라운드 테스트 0건**(OQ-P-252)).
