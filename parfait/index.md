...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `8eb2af7d`(2026-08-22 검증, #329까지 — **고른 배경이 처음으로 남고, 편집 화면이 mock을 버렸다**. C-301 확인이 세 갈래로 갈린다: 색은 `#RRGGBB` PATCH, 기기 사진은 캐시 복사 → 발급 → S3 PUT → confirm 뒤 `imageId` PATCH, 서버에 이미 있던 배경은 요청 0건. **저장이 끝나야 화면을 넘긴다**. 앱이 서버에 쓰는 **두 번째 경로**이고 `api/parfait.md`가 **`done`**(회전 제외 5/5) — OQ-P-173 해소, OQ-P-193·194·199 부분 해소. 같은 화면의 토핑 탭이 **서버 캔버스를 그리기 시작**했고 배치 규칙 셋이 `ToppingGeometry`로 올라가 캔버스 메인·편집 탭·배치 화면이 같은 값을 본다. `NavKeyCanvasBGEdit`가 `data class(groupId, parfaitId)`로 승격되고, `AppError`에 **기기에서 오는 실패** `UnsupportedImage`가 생겼다. ⚠️ **소유 판정이 축이 다른 두 id를 견주는데 이 화면에서는 그것이 곧 편집 게이트다**(OQ-P-250). ⚠️ 마감 409가 "잠시 후 다시"로 접힌다(OQ-P-261) · 업로드 캐시가 쌓이기만 한다(OQ-P-262) · 6자리 HEX 변환 함수가 둘이고 알파 처리가 정반대다(OQ-P-263). 선작성 스펙·플랜이 없어 아카이브 이동 0건이고, C-301 스펙 둘에 as-built 재정정 절이 붙었다. 유닛 696 → **737건**, 클래스 81 → **87개**. 직전 기준선 `a0d584ef`는 #326 — **화면 전환이 처음으로 앱의 결정이 된** 라운드다(오른쪽에서 덮고 오른쪽으로 빠진다. ⚠️ 산출물을 아무도 본 적이 없고 유일한 예외가 붙은 화면은 도달 불가 — OQ-P-260·OQ-P-259). 이월된 위험은 그대로다 — **실기기·실서버 확인 0회**(OQ-P-146, 이제 쓰기 경로가 둘이다)·**다운샘플 상한 미측정**(OQ-P-228)·**Spotlight 라운드 테스트 0건**(OQ-P-252)).
