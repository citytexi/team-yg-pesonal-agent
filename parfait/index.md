...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `95b7fc4d5`(2026-09-10 검증, #487까지 — **선작성 문서가 브랜치 팁 그대로 들어왔는데, 어긋난 자리는 본문이 아니라 문서 둘레에 있었다**. delta 1건, **29파일 2109/458**, 머지 트리 = 브랜치 팁. 유닛 1229 → **1282**(+53)·계측 **39** 유지, **스펙 1·계획 1 아카이브 이동**, 미결 **신설 3건**(OQ-P-399~401, `oq-next` 402). 세그멘테이션 재시도 회복(후보 0건 뒤 「다시 시도」가 전처리 사다리를 한 번 돈다)과 실패 화면 「직접 편집」(C-104 직행) 전환이 들어왔다. 스펙 `related_code` 의 옮기기 전 이름 다섯, navigation-flow 의 「직접 편집」 갈래 누락, OQ 둘의 옛 버튼 서술을 정정했다. ⚠️ 조건부 항목의 철회 근거인 단계 로그가 logcat 밖으로 나가지 않고(OQ-P-399), 실기기 확인은 0회다(OQ-P-400). `api/` 는 원격 연동 코드 0건이라 불변. 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`69bbbe68`, #483까지) 요약은 doc-baseline 본문에 있다).
