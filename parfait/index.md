...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `f31b8c30`(2026-08-23 검증, #335까지 — **앱이 캔버스를 밖으로 내보내고, 처음으로 서버의 것을 지운다**. delta 2건이고 둘 다 머지 트리 = 브랜치 팁, 선작성 스펙·플랜이 없어 아카이브 이동 0건이다. **#324**: 지난 캔버스의 "갤러리에 저장"이 로그 한 줄에서 실제 저장이 됐다(OQ-P-211 해소) — 한 동작이 **MVI 왕복 두 번**으로 갈리고(비트맵은 컴포지션만 만들 수 있다) `YGCanvas`가 캡처 레이어를 **배경·토핑만 담는 안쪽 `Box`**에 걸어 "그림 vs 프레임"을 처음 갈랐다([스펙 신설](specs/archive/2026-08-23-c001-canvas-gallery-save.md)). **#335**: 삭제 확인 모달이 곧 `DELETE`이고 성공해야 목록에서 뺀다 — `parfait-image` 미소비가 셋에서 **둘**로 줄고 OQ-P-199 ③이 닫혔다. ⚠️ **삭제만 즉시 영구인데 실패가 화면에 안 닿는다**(OQ-P-270 — 403·409·404가 로그 한 줄, 같은 화면 배경 저장은 토스트라 처분이 갈렸다). ⚠️ 같은 커밋이 **`TOPPING_MAX_SCALE`을 근거 없이 지웠다**(OQ-P-271). ⚠️ 캡처가 지금 그려진 것을 복사해 배경 로딩 전이면 그대로 담긴다(OQ-P-272) · 얼럿 호스트가 트리거 없이 들어왔다(OQ-P-273) · 저장이 API 29를 경계로 갈린다(OQ-P-274). 계약 문서는 delta가 없어 `verified`·`android_status` 불변이고 소비 현황만 갱신했다. 유닛 737 → **745건**, 클래스 87 유지. 직전 기준선 `96dc215c`는 #339 — **앱이 처음으로 화면 방향을 정했고 그 결정에 시한이 붙은** 라운드다([ADR-0027](adr/0027-portrait-orientation-lock.md), 대화면 opt-out이 `targetSdk 37`에 사라진다 — OQ-P-264 · 세로 고정이 카메라 기준까지 고정했다 — OQ-P-265). 이월된 위험은 그대로다 — **실기기·실서버 확인 0회**(OQ-P-146)·**다운샘플 상한 미측정**(OQ-P-228)·**Spotlight 라운드 테스트 0건**(OQ-P-252)).
