...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `19cb5299`(2026-08-22 검증, #334까지 — **머지 하나에 PR 넷이 들어와 앱이 처음으로 서버에 무언가를 만든다**. C-106 결선 스택 PR3~PR6이 한 머지로 landing했고 커밋 43개·77파일·삽입 3602줄·삭제 433줄이다. 확인 버튼이 발급 → **S3 PUT** → confirm → 배치 네 단계를 태워 **저장 없이 화면만 바뀌던 상태가 끝났다**(OQ-P-238 ② 해소). 테두리는 굽지 않고 서버 필드로 가고(ADR-0025 `accepted`), 흐름 상태는 DataStore 초안 한 벌로 모여 `NavKeyCanvasToppingPlace`가 **인자를 잃었다**(ADR-0026 `accepted`). 배치에 성공한 알맹이는 갤러리 "최근"에 남아 다시 쓰인다 — 흐름의 **두 번째 입구**다. 머지 트리가 PR6 브랜치 팁과 같아(충돌 해소 편집 0건) 네 브랜치의 as-built 수치를 재측정 없이 승격했다. 유닛 602 → **694건**, 클래스 70 → **80개**. `api/image.md`가 `done`이 되며 **표면만 있고 소비처 0인 도메인이 사라졌다**. 남은 위험은 **실기기·실서버 확인 0회**(OQ-P-146 — 이번엔 실패가 서버에 흔적을 남기는 쓰기 경로다)·**다운샘플 상한 미측정**(OQ-P-228)·**Spotlight 라운드 테스트 0건**(OQ-P-252). 직전 기준선 `ef55a58c`는 #311·#325로 **두 삭제 라운드가 지운 근거가 같았던** 라운드).
