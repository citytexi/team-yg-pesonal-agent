...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `d634efd3`(2026-08-23 검증, #336까지 — **편집 결과가 처음으로 남는다, 그리고 남지 않은 것만 조용해졌다**. delta 1건이고 머지 트리 = 브랜치의 develop 병합 커밋이라 충돌 해소 편집 0건, 선작성 스펙·플랜이 없어 아카이브 이동도 0건이다. C-301 편집 탭이 일주일 동안 고친 것을 버리던 화면이었는데 **이동·크기·회전이 확인 버튼에서 PATCH로 나가기 시작했다** — `parfait-image` 미소비가 둘에서 **하나**(테두리 수정)로 줄고 **OQ-P-199가 해소됐다**. 설계 핵심 셋: **바뀐 것만 보낸다**(조회 응답 스냅샷과 대조) · **토핑끼리는 병렬이되 배경 저장보다 완전히 앞**(한쪽만 실패한 경우를 갈라 다루지 않으려고) · **`positionZ`는 안 보낸다**(부분 병합이라 서버 겹침 순서 유지). ⚠️ **실패가 화면에 안 닿는데 확인은 성공한다**(OQ-P-275 — 배경이 성공하면 화면이 넘어가고 캔버스 메인은 옛 좌표를 그린다. 같은 버튼 안에서 배경은 토스트+잔류, 토핑은 무반응+이동이고 마감 캔버스면 같은 409가 두 처분 — OQ-P-261). ⚠️ **다섯 중 테두리 재편집만 혼자 안 나간다**(OQ-P-276, 표시 쪽 OQ-P-254와 같은 값). ⚠️ **어제 지운 배율 상한이 오늘 요청 값이 되고 회귀 테스트로 굳었다**(OQ-P-271). 계약 문서는 delta가 없어 `verified`·`android_status` 불변이고 소비 현황만 갱신했다. 유닛 745 → **751건**, 클래스 87 유지. 직전 기준선 `f31b8c30`은 #324·#335 — **앱이 캔버스를 밖으로 내보내고 처음으로 서버의 것을 지운** 라운드다([갤러리 저장 스펙](specs/archive/2026-08-23-c001-canvas-gallery-save.md) · OQ-P-270 · OQ-P-272~274). 이월된 위험은 그대로다 — **실기기·실서버 확인 0회**(OQ-P-146)·**다운샘플 상한 미측정**(OQ-P-228)·**Spotlight 라운드 테스트 0건**(OQ-P-252)).
