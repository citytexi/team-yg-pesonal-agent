...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `c37dc2b4c`(2026-09-11 검증, #490까지 — **선작성 as-built 가 머지 코드와 한 줄도 어긋나지 않았고, 새로 드러난 것은 코드가 아니라 버전 표식이다**. delta 3건, **8파일 133/3**, 머지 트리 = 브랜치 팁. 유닛 1282 → **1288**(+6)·계측 **39** 유지, 아카이브 이동 0건, 미결 신설 0건(`oq-next` 402). #489 로 카메라·갤러리가 진입할 때 권한을 묻고 CameraX 바인딩이 권한을 기다린다 — 문서 PR #406 이 먼저 붙인 c101·c102 as-built 절과 대조해 어긋난 자리가 0건이라 미머지 표기만 걷었다. #488·#490 으로 1.1.3 이 코드 10·11 두 벌이 됐고 경량 태그 `1.1.3` 은 11 만 가리킨다(OQ-P-310). `api/` 는 원격 연동 코드 0건이라 불변. 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`95b7fc4d5`, #487까지) 요약은 doc-baseline 본문에 있다).
