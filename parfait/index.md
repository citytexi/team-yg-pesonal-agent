...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `23675cc1`(2026-09-08 검증, #466까지 — **선작성 문서 한 벌이 통째로 들어왔고, 버그픽스 둘이 문서가 단정한 적 없는 자리를 건드렸다**. delta 3건, **39파일 2102/782**, 머지 셋 다 트리 = 브랜치 팁. 유닛 1106 → **1137**, 계측 35 → **37**, **선작성 스펙 1·계획 1 아카이브 이동**(`topping-border-distance-field`). #464는 거리판 코어를 `core:util:{jvm,android}`·`core:ui` 에 세워 네 화면을 같은 거리장으로 모았고, **스펙과 어긋난 자리는 0건·계획과는 넷**이다(값 타입이 `model/` 로 갈라짐 · 예정에 없던 `ToppingBorderPlateCache` · **지연 재생성 폐기** · 굵기 2~30dp `domain` 집중). 그 굵기 클램프가 읽는 쪽 매퍼에 들어가 **`api/conventions.md` 「Android 불일치」가 3 → 4건**이 됐다 — **앱이 스스로 만든 첫 간극**이다. #465는 캔버스보다 커진 토핑에서 박스와 핸들이 벌어지던 것을 `Modifier.centeredAt` 통일로 고쳤다(원인은 값이 아니라 **놓는 방식** — 잘린 겉크기 안 가운데 정렬 때문에 넘침의 절반만큼 밀린다). #466은 초대코드 입력을 텍스트 필드 하나로 합쳤고 문서는 브랜치 때 이미 반영돼 꼬리표만 정정했다. 미결 **신설 0건**(`oq-next` 383 유지)·OQ-P-102에 📌 1덩이, **미머지 표기 스무 자리 제거**. 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`2285d09d`, #463까지) 요약은 doc-baseline 본문에 있다).
