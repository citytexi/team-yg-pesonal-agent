...
본인 토핑은 Spotlight에서 빠진다. 다만 **C-305 진입은 여전히 `TODO`**라 그 탭은 무반응이다(OQ-P-250 ③). 작성자 칩도 서버 필드가
...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `4da18230`(2026-08-27 검증, #363까지 — **세 라운드째 "미머지"로 세어 온 브랜치들이 마침내 develop 으로 들어왔다. 다만 release 가 받은 그 커밋들이 아니다**. delta 1건인데 커밋 44개·20파일 3,685/223이고 **선작성 스펙 셋**(마스크 후처리·알파 정련·커널 취소 확인 전환)이 한 머지에 실렸다. 머지 트리 = 정련 브랜치 팁이라 충돌 해소 편집 0건, 유닛 819 → **926**(+107)·계측 14 유지·아카이브 이동 6건(스펙 3·계획 3). **누끼가 처음으로 모델 출력을 고친다** — 알파 후처리 커널(이진화 축소·area opening·팽창·keep 적용·가이드 필터 정련·침식)이 붙고, 후보 판정이 사각형 면적에서 **알파 커버리지**로, 중복 제거가 **IoU 병합**으로 옮겼다. 취소 확인은 `checkCancelled` 콜백에서 `suspend` + `job.ensureActive()` 로 바뀌었다(⚠️ `suspend` 가 CPU 루프의 취소를 만들어 주지는 않는다 — OQ-P-318). **미머지 추적 항목 0**이 됐으나 release 와의 격차는 **43커밋 그대로이고 내용까지 갈렸다**(develop 은 rebase 된 다른 커밋을 받았다) → OQ-P-311 ①②. ⚠️ **임계·반경·정칙화를 판정할 실기기 사진 세트는 0회**(OQ-P-287~300). 신규 미결 2건(OQ-P-318·319). 직전 회차(`5dc9fbec`, #389까지) 요약은 doc-baseline 본문에 있다).
