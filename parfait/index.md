...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `a5e8a760`(2026-08-25 검증, #349까지 — **촬영이 최대 품질로 들어오기 시작했고, 누운 사진이 세워졌다**. delta 1건이고 머지 트리 = 브랜치 팁이라 충돌 해소 편집 0건. **선작성 스펙이 부분만 머지돼 아카이브 이동이 0건인 라운드** — 근거 등급으로 갈린 스펙의 **1단계만** 들어와 스펙은 `in-progress`, 계획은 active에 남는다. 손실의 첫 세대(`ImageCapture` 기본 빌더가 JPEG로 찍던 자리)와 API 28 미만 EXIF 회전, 그리고 최근 이미지 확장자의 바이트 판정이 들어왔고 **PNG 저장·512 확대·API 28 이상 회전은 사진 세트 측정이 게이트라 미착수**다. 테스트 775 → 781건. **미머지 추적 항목 넷**(후처리 스택 셋 + 알파 정련 하나)).
