...
서버 계약은 `api/`에 스냅샷돼 있다(도메인 8건·**엔드포인트 30개 + 테스트 전용 1**, 서버 `09e7d92`).
...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `acbc4b45`(2026-09-09 검증, #473까지 — **하루를 묵힌 선작성 문서 두 벌이 나란히 들어왔고, 둘이 같은 그림의 크기를 반대 방향에서 건드린다**. delta 2건, **21파일 941/68**, 머지 둘 다 트리 = 브랜치 팁. 유닛 1139 → **1164**, 계측 **37** 유지. **선작성 스펙 2·계획 2 아카이브 이동**(자동 Task 전량 수행, 실기기 수동 검증 Task 만 미체크), 미결 **신설 5건**(OQ-P-387~391, `oq-next` 386 → 392). #472 는 커버리지 하한을 `data` 의 `internal` 에서 `domain` 의 `SubjectCoverage` 로 올려 자동 후보 필터·저장소 알파 정제·편집 화면이 **같은 함수**를 보게 했다 — 새 기준을 만들지 않은 것이 핵심이고, 자동으로 버려질 크기를 수동 편집으로 만들 수 없게 된다. 측정은 한 번의 스캔(`measureSubject` + `trimTo`)이고 차단은 파일을 쓰기 전이다. `borderOnly` 진입만 판정에서 빠졌는데 **그 예외는 구현이 먼저 드러내고 스펙이 뒤따라 적었다**. #473 은 업로드 경계에 전처리를 세워 파일·포맷을 **쌍으로** 발급과 PUT 에 넘기고, 판정(`UploadImagePlan.of`, 순수)과 실행(`UploadImagePreprocessorImpl`)을 갈라 결정 표를 JVM 유닛으로 덮었다. ⚠️ **없던 디코드가 생겼다** — 변경 전 업로드는 바이트 복사뿐이었다. 저장소 주석 하나가 **스펙이 철회한 메모리 비교**를 그대로 들고 있다(OQ-P-388, 동작 영향 없음). **두 라운드가 만나는 자리가 이번의 발견**(OQ-P-391) — 하한은 편집본 원본 해상도에서 재는데 업로드본은 긴 변 1500까지 줄어, 하한을 어느 좌표계에서 재는지가 정해진 적이 없다. `api/` 는 원격 연동 코드 0건이라 불변. 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`b7674e88`, #470까지) 요약은 doc-baseline 본문에 있다).
