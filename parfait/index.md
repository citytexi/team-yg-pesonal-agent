...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `750cc2dd`(2026-08-20 검증, #310까지 — **미머지 스택 셋(#307·#308·#310)이 한 번에 들어와 계약 정합이 처음으로 0건이 됐다**. 그룹 목록·상세가 `:data` 인메모리 캐시 한 벌로 모이고(ADR-0023 `accepted`), 칩 배정 주체가 서버로 정해져 세 화면의 인덱스 순환 mock이 사라졌으며, `GroupDetailVO`와 `XxxVOMapperTest`가 함께 소멸했다. 남은 위험은 **재발 방지 수단 없이 develop이 네 자리에서 서버 칩 키로 색을 정한다**는 것(OQ-P-234 ③). 직전 기준선 `c36cad49`는 #306 회원 탈퇴 결선).
