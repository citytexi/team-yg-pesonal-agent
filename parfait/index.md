...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `2285d09d`(2026-09-07 검증, #463까지 — **버그픽스 둘이 들어왔고, 하루 전에 쓴 선작성 문서 둘 중 하나만 낡았다**. delta 2건, **14파일 302/28**. 유닛 1096 → **1106**, 계측 **35** 유지, **선작성 스펙 1·계획 1 아카이브 이동**. 머지본이 **스펙과는 어긋난 자리 0건, 계획과는 세 자리**에서 갈렸다 — 앞 회차가 코드리뷰 결과를 스펙에만 실었기 때문이고, 이번 감사의 절반이 계획에 as-built 각주를 다는 일이었다. #463은 캡처 홀더로 저장 미리보기의 디스크 왕복을 없앴고(비우는 자리는 `onDispose`의 백스택 조건 하나), #461은 참여 갈래 닉네임 초기값을 **생성 갈래의 경로를 복제해** 채웠다. 값이 없을 때 두 갈래가 반대로 답하는 것이 미결 1건 신설(OQ-P-377), 미머지 표기 셋 제거·2건 정정(364·365). 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`5907e286`, #458까지) 요약은 doc-baseline 본문에 있다).
