...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `cbb48cd8`(2026-08-26 검증, #366까지 — **바이너리 넷이 바뀐 라운드라 무엇이 바뀌었는지를 커밋 메시지 말고는 아무도 말해 주지 않는다**. delta 1건·5파일 97/0 + `.ttf` 넷 교체이고 머지 트리 = 브랜치 팁이라 충돌 해소 편집 0건, **`.kt` 0줄 변경이라 유닛 785·계측 14건 유지·아카이브 이동 0건·코드 심볼 드리프트 0건**. SUIT 원본의 빈 글리프 cmap 매핑 탓에 투명하게 찍히던 문자를(#365) 매핑만 걷은 수정본으로 갈아 시스템 폰트 fallback이 걸리게 했다. ⚠️ 대가 둘 — 기기마다 글자체가 섞이는데 실기기 확인이 없고, OFL 1.1로 생긴 `OFL.txt`가 APK에 안 실리며 앱에 오픈소스 고지 화면도 없다. ⚠️ 바이너리라 **회귀 감지선이 0건**이다. 신규 미결 둘(OQ-P-306·307). **미머지 추적 항목 넷** 그대로).
