...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `27e85d0d`(2026-08-30 검증, #409까지 — **배포 계보가 develop 을 앞질렀다**. delta 4건(#405 스포트라이트 토스트 교체 · #406 갤러리 상단바 Title · #407 첫 조회 로딩 · #409 앱 버전 0.1.1), 14파일 383/59, **네 머지 모두 트리 = 브랜치 팁**(충돌 해소 편집 0건). 선작성 스펙·플랜이 없어 **아카이브 이동 0건**, 유닛 942 → **949**(+7, 파일 100 유지)·계측 14 → **17**(+3, `YGToastHostTest` 신설). **토스트 스택에 교체 갈래가 생겼다** — `show(type, replaceTag)` 가 같은 태그를 걷어내고, 태그를 주는 곳은 캔버스 Spotlight 하나다(태그 문자열이 화면 소유라 서로 지울 수 있다 → OQ-P-329). **`YGFloatingBarTitle` 이 들어와 변형이 5종**이 되고 C-102 갤러리가 수제 상단바를 버렸다 — 두 회차째 "develop 미머지"로 세던 항목이 닫혔고 닫기 버튼에 접근성 레이블이 따라왔다(빈 상태 제목 부재의 근거는 작업자 지시뿐 → OQ-P-331). **로딩 오버레이가 첫 조회로 넓어졌다** — G-001·C-001 이 `YGScaffoldV2(isLoading)` 을 켜되 **"아직 한 번도 못 받은 조회"**일 때만이고, 판정이 두 ViewModel 에 복제됐다(OQ-P-330, OQ-P-205 ②). ⚠️ **OQ-P-311 의 갈림이 뒤집혔다** — release 계보가 둘 늘어 최신 기준 **release 만 50커밋 · develop 만 8커밋**이고, `debug-mode`·`cache-image`·`canvas-polling` 이 release 에만 있어 **선작성 문서 넷이 구현이 끝난 채 `draft` 로 남는다**(③ 재개). PR2 계획의 `loadTodayCanvas()` 교체 블록이 첫 조회 덮개를 모르는 것은 OQ-P-326 ⑥. 직전 회차(`627e1867`, #397까지) 요약은 doc-baseline 본문에 있다).
