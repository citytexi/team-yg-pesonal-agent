...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `1b21725ba`(2026-09-11 검증, #496까지 — **로컬 커밋 단계에서 먼저 적은 문서가 머지 코드와 맞았는데 사본 수를 하나 잘못 셌고, 직전 회차가 적은 릴리즈 브랜치는 이미 원격에서 지워져 있었다**. delta 1건, **14파일 151/28**, 머지 트리 = 브랜치 팁. 유닛 1288 → **1292**(+4)·계측 **39** 유지, 아카이브 이동 0건, 미결 신설 0건(`oq-next` 402). #496 으로 그룹 목록 응답의 테두리 세 필드를 `MyParfaitGroupVO.recentImageBorder` 가 받고, 캔버스·토핑 매퍼의 테두리 변환 사본 둘이 공용 `ToppingBorderMapper.kt` 하나로 모였다(그룹 매퍼가 세 번째 호출부) — `api/` Android 쪽 서술(parfait-group·conventions·parfait-image·README)과 OQ-P-316 을 머지로 고치고 module-structure·data-layer 에 두 번째 공용 매퍼를 적었다. 렌더는 별도 티켓이라 OQ-P-316 은 열린 채다. 원격 `release/*` 가 0개이고 코드 10 을 가리키는 ref 도 0개다(OQ-P-310·311). 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`c37dc2b4c`, #490까지) 요약은 doc-baseline 본문에 있다).
