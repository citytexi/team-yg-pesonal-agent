...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `f37a76540`(2026-09-16 검증, #499까지 — **선작성 문서 두 벌이 같은 날 들어왔는데 한 벌은 한 줄도 어긋나지 않았고 다른 한 벌은 구현이 스펙보다 두 자리 넓었다**. delta 2건, **16파일 1649/29**, 머지 둘 다 트리 = 브랜치 팁. 유닛 1292 → **1298**(+6)·계측 39 → **46**(+7), 아카이브 이동 **4건**(스펙 2 · 계획 2), 미결 신설 1건(OQ-P-402, `oq-next` 403). #497 로 G-001 목록 토핑이 테두리를 그리기 시작했고(`YGToppingImage.Remote.border`, 알맹이를 굵기만큼 줄여 96dp 프레임 유지) 목록과 캔버스가 열쇠를 공유하는 탓에 `ToppingBorderPlateCache` 가 열쇠당 **크기별 최대 3장의 선반**이 됐다 — 스펙과 어긋난 조항 0건이라 점검은 대조가 아니라 「머지 후 문서 반영」 절을 닫는 일이었다(design-system·ADR-0030·OQ-P-316 렌더 해소·OQ-P-317). #499 의 빌드 캐시 측정 하니스는 `tools/build-cache-bench/` 에만 있어 앱 코드에 닿지 않지만 **구현이 스펙보다 넓어** (`check-relocatability.sh`·`report.py`/`report.html`, `--pair <A>:<B>`) 스펙 본문 다섯 절을 머지본 기준으로 고쳤다. ⚠️ **하니스만 머지됐고 측정·게이트 G0 는 아직 아무도 돌리지 않았다** — 도입 문턱값과 커밋 쌍 선정은 OQ-P-402 가 잇는다. 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`1b21725ba`, #496까지) 요약은 doc-baseline 본문에 있다).
