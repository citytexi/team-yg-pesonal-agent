...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `2b1dce3a`(2026-09-04 검증, #451까지 — **앱 코드는 한 줄도 안 바뀌었는데 서버 계약을 손으로 확인할 자리가 둘 생겼다**. delta 1건(#451 API 현행화 — `http/` 요청 모음), **5파일 550/3**. 유닛 **1029**·계측 **17** 유지, 아카이브 이동 0건, 미결 1건 신설(OQ-P-354)·4건 갱신(092·108·341·352). `notifications.http`(기기 토큰 등록)와 `fcm-test.http`(**서버를 거치지 않고 FCM 으로 직접** 발송)가 들어와 `http/` 커버가 **25/29 → 26/29**, 일곱 번째 왕복인데 **방향이 반대다** — 요청 모음이 앱 코드보다 앞서 나갔다(등록을 부를 수단이 develop 에 0건). `.kt`·gradle 무변경이라 **코드 드리프트 0건**. ⚠️ 대신 `fcm-test.http` 가 서버 발송 페이로드(문구·`data` 키·채널 id·TTL)를 상수로 복제했는데 **그 갈라짐을 세는 축이 없다** → OQ-P-354. 두 파일 다 실행 기록 0건. 미머지 브랜치는 다섯에서 **일곱**으로 늘었다(알림 권한·캔버스 튜토리얼 신규 — 권한 브랜치가 OQ-P-341 ②③④를 답한다). 직전 회차(`c74f40eb`, #444까지) 요약은 doc-baseline 본문에 있다).
