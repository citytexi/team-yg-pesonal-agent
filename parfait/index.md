...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `37ea970b`(2026-08-25 검증, #350까지 — **세 주 전에 "사라졌다"고 적어 둔 결함이 다른 갈래에 살아 있었다**. delta 1건이고 머지 트리 = 브랜치 팁이라 충돌 해소 편집 0건, **선작성 스펙·플랜 없는 버그 라운드라 아카이브 이동 0건**이며 테스트 781건 유지. 갤러리 권한 화면이 Route가 준 `innerPadding` 위에 `windowInsetsPadding(systemBars)`을 한 번 더 걸어 닫기 버튼이 상태바 높이만큼 내려앉아 있었고(#345), 스펙이 2026-08-04에 적은 "인셋 이중 적용이 사라졌다"는 **목록 갈래에만** 참이었다 — **인셋 소유는 화면이 아니라 갈래 단위로 갈린다**. 카메라는 기전이 반대라 **무는 자리**만 옮겼고, PR 요약 밖에서 가이드 토스트가 권한 게이트를 얻었다. ⚠️ **실기기 확인 0회**(OQ-P-301). **미머지 추적 항목 넷** 그대로).
