...
C-301 편집 탭(`CanvasBGEditViewModel`) 하나다.
...
`false`라 본인 토핑도 Spotlight로 들어가고 C-305 진입은 `TODO`다(OQ-P-250). 작성자 칩도 서버 필드가
...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `a0d584ef`(2026-08-22 검증, #326까지 — **화면이 어떻게 바뀌는지가 처음으로 앱의 결정이 됐다**. 라이브러리 기본(페이드+축소)에서 **오른쪽에서 덮고 오른쪽으로 빠지는** 밀기로 바뀌었고, `NavTransition`이 `push`·`pop`·`predictivePop`을 한 값으로 묶어 `metadata`로 화면별 override를 연다(붙는 대상은 **위에 놓이는 화면**이다). ⚠️ **산출물을 아무도 본 적이 없다** — 테스트는 세 슬롯이 비지 않았다는 것만 잠그고, 유일한 예외 `Fade`가 붙은 화면은 짝과 함께 **도달 불가**라 그 근거인 공유 요소 전환이 실행되지 않는다(OQ-P-260·OQ-P-129 ②). 앱 기본을 무는 세 줄이 `MainRoute`·app-preview `RootRoute` **두 곳에 복제**됐다(OQ-P-259). 선작성 스펙·플랜이 없어 아카이브 이동 0건이고, 문서 결과는 **최종 커밋이 지운 KDoc을 [navigation-flow](architecture/navigation-flow.md) 「화면 전환」으로 받아 낸 것**이다. 유닛 694 → **696건**, 클래스 80 → **81개**. 직전 기준선 `19cb5299`는 #334 — **머지 하나에 PR 넷이 들어와 앱이 처음으로 서버에 무언가를 만든** 라운드다(확인 버튼이 발급 → S3 PUT → confirm → 배치 네 단계를 태우고, 테두리는 서버 필드로, 흐름 상태는 DataStore 초안으로 갔다 — ADR-0025·ADR-0026 `accepted`). 이월된 위험은 그대로다 — **실기기·실서버 확인 0회**(OQ-P-146, 이제 실패가 서버에 흔적을 남기는 쓰기 경로다)·**다운샘플 상한 미측정**(OQ-P-228)·**Spotlight 라운드 테스트 0건**(OQ-P-252)).
