...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `36719e8e`(2026-08-20 검증, #318까지 — **두 PR이 각각 "임시"라고 적어 둔 자리를 결정으로 바꿨다**. #315는 온보딩 약관의 실패 표현 둘을 확정하며 스캐폴드 V1 잔여를 2파일로 줄였고(이관 17화면), #318은 하루 전 서버 409 가드로 거짓이 된 앱 주석 일곱을 고치고 `ServerErrorCode.Parfait`를 신설했다. 머지 커밋 둘 다 트리가 브랜치 팁과 같다(충돌 해소 편집 0건). 유닛 560 → 561건. 남은 위험은 **다운샘플 상한 미측정**(OQ-P-228)과 **실기기·실서버 확인 0회**(OQ-P-146). 직전 기준선 `cf357937`은 #309로 미머지 스택이 비고 크래시 셋이 닫힌 라운드).
