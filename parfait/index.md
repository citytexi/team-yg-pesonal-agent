...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `c55e10bc`(2026-08-26 검증, #376까지 — **문서가 세 회차째 "미머지"로 세어 온 넷이 develop이 아닌 곳에서 이미 배포됐다**. delta 3건·13파일 144/54, 세 머지 전부 트리 = 브랜치 팁이라 충돌 해소 편집 0건, 아카이브 이동 0건, 유닛 785 → **789건**·계측 14건 유지. ⚠️ **#374의 `versionCode` 결번 하나(2)가 실마리였다** — 태그 `0.0.3`이 가리키는 `release/version-0.0.3-3`이 develop에 없는 45커밋을 담고 있고 그중 넷이 그 세그멘테이션 브랜치들이다(OQ-P-311, 감사 대상을 어떻게 둘지가 다음 회차의 첫 결정). **#372**로 릴리즈가 처음 실제로 조립되며(릴리즈에서만 터지는 lint 실패를 고쳤다) **리소스 축소가 처음 켜졌고** crashlytics-ndk가 붙었다 — ⚠️ 둘 다 산출물을 실행해 본 기록이 0건(OQ-P-308·309). **#376**이 서버 `ownerType`을 **같은 날** 읽어 `CanvasToppingVO.isMine`으로 접으며 C-202의 상수 `false`와 C-301의 축이 다른 비교를 함께 없앴다(OQ-P-250 ②해소·③잔존). 신규 미결 넷(OQ-P-308~311)).
