...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `bf06c830`(2026-08-26 검증, #371까지 — **여섯 날 전의 결정이 되돌려졌고, 그 결정을 적어 둔 문서 한 줄이 그동안 거짓이었다**. delta 1건·4파일 37/3이고 머지 트리 = 브랜치 팁이라 충돌 해소 편집 0건, **신규 테스트 0건**이라 유닛 789·계측 14건 유지·아카이브 이동 0건. 카메라·갤러리가 `YGToastHost`를 다시 자기 프레임 안으로 가져갔다 — #309가 걷어 스캐폴드로 올렸던 형태로 복귀했고, 스캐폴드 호스트가 상태바 아래에 떠서 헤더 행을 덮었기 때문이다. **이 회차가 고친 것은 코드 드리프트가 아니라 c102·c101 스펙이 여섯 날 전에 스스로 적은 단정 둘**("보이는 위치는 사실상 그대로다"·"위키 공통 정책에는 이쪽이 맞는다")이다. ⚠️ 토스트 관용구가 셋이 됐는데 규칙이 없고, 발행할 수 없는 호스트가 화면마다 하나씩 생기며, 호스트가 권한 허용 갈래 안에만 있다(OQ-P-312 신설). 미머지 넷 그대로이고 release 브랜치와의 격차는 45 → **43커밋**. 직전 회차(`c55e10bc`, #376까지) 요약은 doc-baseline 본문에 있다).
