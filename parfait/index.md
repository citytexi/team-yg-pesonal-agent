...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `b7674e88`(2026-09-08 검증, #470까지 — **어제 벌어진 간극을 오늘 닫았는데, 닫힌 것은 계약 표이지 호출 수가 아니다**. delta 2건, **12파일 57/6**, 머지 둘 다 트리 = 브랜치 팁. 유닛 1137 → **1139**, 계측 **37** 유지. 선작성 문서가 없어 **아카이브 이동 0건**, 미결 **신설 0건**(`oq-next` 386 유지). **`api/conventions.md` 「Android 불일치」가 5 → 4건**이 됐다 — 서버 `9c13852` 가 벌린 행이 하루 만에 걷혔다. #469 는 `GetTodayParfaitResponse`·`CanvasVO` 에 `groupName` 을 세워 오늘·상세 두 조회가 같은 매퍼로 이름을 나르게 했지만, **정본은 그룹 목록 캐시(ADR-0023)로 남겼다** — 캔버스가 준 이름은 그 캐시가 빌 때(푸시 딥링크·프로세스 재시작 복귀)만 상단 바를 채운다. 캔버스 SSoT 가 그룹의 값을 정본으로 들면 ADR-0029 의 "값을 얻는 길은 하나"가 저장소 사이에서 깨지기 때문이고, 그래서 **소유 경계는 응답에 무엇이 실렸는가가 아니라 어느 저장소가 그 값을 책임지는가로 긋는다**(ADR-0029 📌 신설). OQ-P-383 은 **부분 해소**(④ 404 `GROUP_NOT_FOUND` 미분기 잔존). #470 은 버전만 올린 커밋이다(`appVersionCode` 8 · `appVersionName` 1.1.1). 미머지 하나(`feature/debug-mode`) 유지. 직전 회차(`23675cc1`, #466까지) 요약은 doc-baseline 본문에 있다).
