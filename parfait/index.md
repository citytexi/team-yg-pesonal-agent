...
서버 계약은 `api/`에 스냅샷돼 있다(도메인 7건·**엔드포인트 28개 + 테스트 전용 1**, 서버 `bd18af4`).
...
- **[`doc-baseline.md`](doc-baseline.md)** — 문서를 마지막으로 검증한 `develop` 커밋 해시(SoT) + "develop 기준 문서 점검" 절차. 현재 기준선 `df3f4cbe`(2026-08-26 검증, #368까지 — **문서가 미결로 적어 둔 자리 넷이 하루에 닫혔고, 그중 하나는 문서가 못 박은 순서를 뒤집고 닫혔다**. delta 7건·17파일 383/153이고 **일곱 머지 전부 트리 = 브랜치 팁**이라 충돌 해소 편집 0건, **선작성 스펙·플랜 없는 라운드라 아카이브 이동 0건**, 유닛 781 → 785건·계측 12 → 14건. #358 HTTPS 전환(OQ-P-076 해소, `debug-overrides`가 순서 역전의 위험을 흡수) · #352 툴팁 0건 조건 결선(死필드가 정본이 되고 미조회 `null`을 0건으로 안 센다) · #351 배경 폴백이 화면에서 컴포넌트로(값도 `Gray100` → 흰 바탕, ⚠️ 빈 안내판이 배경에 종속돼 정책 근거 없음) · #357 배치 화면이 진짜 캔버스를 그린다(OQ-P-240 해소, ⚠️ 새 경로 테스트 0건) · #354 서명 키 부재 안내(⚠️ 발화 미확인) · #353 약관 각짐 · #368 keystore gitignore. 신규 미결 셋(OQ-P-303·304·305). **미머지 추적 항목 넷** 그대로).
