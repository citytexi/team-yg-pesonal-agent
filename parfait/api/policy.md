...
랜딩 URL이 전부 응답 값이다 → [intro-term-agree 스펙](../android/specs/archive/2026-07-22-intro-term-agree.md).
...
NavKey 통합의 근거이기도 하다 → [navigation-flow](../android/architecture/navigation-flow.md).
...
- **설정 화면은 목록을 "여는 재료"로만 쓴다**(#296) — 약관 두 줄은 `strings.xml`로 고정돼 있어 조회가
  ...
  실패와 무반응이 구분되지 않는다 → [open-questions](../android/synthesis/open-questions.md).
...
- `policies[].url`이 `Tos.content`(LONGTEXT) 컬럼 재사용이라 값이 URL인지 약관 전문인지 스키마로는
  보장되지 않는다. 운영 데이터 투입 규약 확인 필요 → [open-questions](../android/synthesis/open-questions.md)
