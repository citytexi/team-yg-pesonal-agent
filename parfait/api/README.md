...
| [notification.md](notification.md) | `http/notification` | 1 (**기기 FCM 토큰 등록**) + **HTTP 밖 푸시 발송 1종**(토핑 등록) | 구현됨(**등록 표면만** — 호출부 0. **수신·채널·딥링크는 2026-09-05에 생겼고**(PR #446·#447) 토큰을 얻는 `onNewToken`도 돌아왔는데 **등록을 부르지 않는다**) |

**총 30 엔드포인트 + 테스트 전용 1**(2026-09-04, 서버 `aa9cc9b` — 여덟 번째 도메인 `notification`이
2026-09-02에 들어와 29 → 30이 됐고, **이번 회차의 엔드포인트 증감은 0이다**).
...
`parfait_default`를 앱이 그대로 따랐다. **다만 등록 호출부는 여전히 0건**이라 발송은 계속
`NO_DEVICE_TOKEN`으로 취소된다. 즉 이 도메인은 **양끝이 다 서 있는데 가운데 한 줄(토큰 등록 호출)이
비어 통하지 않는 상태**다 → [notification.md](notification.md) "푸시 수신·딥링크",
[open-questions](../synthesis/open-questions.md) OQ-P-341 ②.
