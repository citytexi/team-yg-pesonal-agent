...
| [parfait.md](parfait.md) | `http/parfait` | 5 + 테스트 전용 1 (연도 리스트 · 오늘의 캔버스 · 과거 목록 · **상세 조회** · **배경 변경** / 테스트 회전) | **결선됨**(회전 해당 없음, 5 전부 호출부 있음, **불일치 1건** — 2026-09-07 신설 `groupName`을 앱이 안 읽는다) |
...
**총 30 엔드포인트 + 테스트 전용 1**(2026-09-08, 서버 `09e7d92` — 여덟 번째 도메인 `notification`이
...
상단에 그릴 이름을 얻으라는 것이 근거다. **앱은 아직 이 필드를 읽지 않는다**(OQ-P-383)
