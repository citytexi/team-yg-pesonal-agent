...
| 구분 | repo | 경로 | 용도 |
|------|------|------|------|
| AI 스킬·위키 (여기) | `citytexi/team-yg-pesonal-agent` (public) | `~/Documents/work_station/mashup/team-yg-pesonal-agent` | 위키(`wiki/`)·원본(`raw/`)·AI 스킬·작업 지시 문서 |
| 코드 프로젝트 | `mash-up-kr/TJYG-Android` | `~/Documents/work_station/mashup/github/TJYG-Android` | 실제 Android 앱 코드 (Kotlin 멀티모듈) |

민감 개인정보는 여기 repo의 private submodule(`wiki/personal-private/` → `team-yg-pesonal-agent-privacy-data`)에 둔다.

## 작업 방식

- **코드 작업 대상은 항상 `TJYG-Android`** (`~/Documents/work_station/mashup/github/TJYG-Android`).
